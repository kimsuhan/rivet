...
| 템플릿 역할의 프로젝트 필수 | `initial_role IS NULL OR project_id IS NOT NULL` 검사 제약과 workspace 복합 외래 키 |
...
- 이슈 템플릿 관리의 관리자 권한, version과 활성 이름 유일성, 현재 활성 라벨·프로젝트·역할 팀을 검증
...
| CSV 가져오기 | 워크스페이스 잠금, 검증 실행 claim, 새 라벨·프로젝트·프로젝트 역할 팀, 이슈·팀 작업·라벨 연결, source row, 가져오기 활동, 이슈·팀 표시 번호 카운터와 실행 성공 결과. 한 단계라도 실패하면 전체 롤백 |
| 이슈 템플릿 관리 | 현재 version 조건부 수정·보관, 라벨 연결 교체와 활성 이름 유일성 확인. 영향 행이 없으면 최신 version을 반환하고 덮어쓰지 않음 |
| 템플릿 기반 이슈 생성 | 템플릿 ID·version·보관 상태와 현재 라벨·프로젝트·역할 팀을 다시 확인한 뒤 일반 이슈 생성 스냅샷만 저장. 템플릿 관계와 본문 이벤트 복제 없음 |
