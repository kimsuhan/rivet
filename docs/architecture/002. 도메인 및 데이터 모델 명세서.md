...
| 항목 | 내용 |
| --- | --- |
| 문서 상태 | 확정 — 후속 기술 설계 기준 |
| 문서 버전 | v1.1 |
| 작성일 | 2026-07-10 |
| 대상 릴리스 | Rivet MVP |
| 선행 문서 | [제품 요구사항 정의서](../planning/002.%20제품%20요구사항%20정의서.md), [사용자 흐름 및 화면 명세서](../planning/003.%20사용자%20흐름%20및%20화면%20명세서.md), [기술 아키텍처 명세서](./001.%20기술%20아키텍처%20명세서.md) |
...
이 문서의 모델은 API 응답 구조가 아니다. 데이터베이스 모델을 API에 그대로 노출하지 않으며, 화면과 외부 계약에 필요한 요청·응답 구조는 후속 API 명세서에서 정의한다.
...
- 모든 워크스페이스 업무 테이블은 조회 경계를 명시하기 위해 `workspace_id`를 직접 가진다. 사용자 프로필 범위 파일만 예외적으로 `workspace_id = NULL`을 허용한다.
...
인증 세션, 일회용 토큰, 멘션, 사용자 프로필 사진 참조, 이메일 발송 기록과 내보내기 기록은 핵심 업무 관계를 읽기 어렵게 만들지 않도록 도식에서 생략했다.
...
| `NotificationType` | `ASSIGNED`, `MENTIONED`, `COMMENTED`, `ISSUE_TERMINATED`, `HANDOFF_CREATED`, `HANDOFF_FOLLOWED_UP` | MVP 인앱 알림 유형 |
...
같은 워크스페이스와 정규화 이메일에는 처리 중인 초대를 하나만 둔다. 수락할 때 로그인 이메일 일치, 기존 멤버십 부재와 초대 유효성을 잠금 후 확인하고 멤버십을 한 번만 생성한다.
...
관계는 선행 작업이 완료·취소되어도 삭제하지 않는다. 선행 상태가 `Completed`, `Canceled` 범주이면 화면에서 `차단 해제됨`으로 계산한다.
...
설명 저장 시에는 해당 이슈의 `comment_id IS NULL` 멘션 집합을, 댓글 수정 시에는 해당 댓글의 멘션 집합을 Markdown 파싱 결과와 한 트랜잭션에서 맞춘다. 같은 본문에서 같은 멤버를 여러 번 멘션해도 관계와 알림은 한 건이다.
...
작업 전달은 수정·삭제하지 않는 추가 전용 데이터다. 이슈별 `INITIAL`은 최대 한 건이고 `FOLLOW_UP`은 최초 전달 뒤에만 추가한다. API 명세 링크와 변경 요약 등 템플릿 제목만 있고 실질 내용이 없는 본문은 유효한 전달로 인정하지 않는다.

미완료 웹 또는 앱 역할 팀 작업을 차단하는 백엔드 역할 팀 작업을 완료할 때는 유효한 최초 전달의 존재를 같은 트랜잭션에서 확인한다. 전달 생성, 백엔드 상태 변경, 활동과 Outbox 발행은 함께 커밋한다.
...
업무 변경과 Outbox 삽입을 같은 트랜잭션에 둔다. 워커는 `FOR UPDATE SKIP LOCKED`로 처리할 행을 가져가고 제한된 지수형 재시도를 사용한다. `last_error_code`에는 정제된 코드만 저장하고 요청·응답 본문과 비밀 값을 넣지 않는다.
...
`(event_id, recipient_membership_id)`를 유일하게 해 멘션·구독·담당·전달 규칙이 겹쳐도 수신자별 한 건만 만든다. 행위자와 수신자가 같거나 수신 멤버십이 비활성이면 생성하지 않는다. 알림 본문 스냅샷은 저장하지 않고 유형과 현재 이슈 정보로 한국어 요약을 만든다.
...
- 후행 프론트 작업이 있는 백엔드 작업 완료 시 최초 전달 존재
...
| 차단 관계 변경 | 양쪽 잠금, 순환 확인, 관계, 양쪽 활동·버전, Outbox |
| 댓글 저장 | 댓글, 멘션 집합, 댓글 이미지 연결과 `unlinked_at`, 자동 구독, 활동, Outbox |
| 백엔드 작업 완료 | 최초 전달 검증·생성, 전달 이미지 연결과 `unlinked_at`, 상태 변경, 활동, Outbox |
| 추가 전달 | 추가 전용 전달, 전달 이미지 연결과 `unlinked_at`, 활동, Outbox |
| 프로필 사진 변경·삭제 | 새 파일 소유·범위 검증, `avatar_file_id` 교체 또는 해제, 새·이전 파일의 `unlinked_at` 변경 |
| 휴지통 이동·복구 | 삭제 시각, 노출 상태, 활동, 예약 Outbox 생성·취소 |

외부 메일 발송, 알림 수신자 계산, SSE 전송, PostHog 전송과 실제 파일 삭제는 주 트랜잭션 안에서 실행하지 않는다. 파일시스템과 데이터베이스 사이의 실패 구간은 기술 아키텍처의 임시 저장·원자 이동·정기 정합성 작업으로 보완한다.
...
| 작업 전달 | 미완료 프론트 후행 작업이 있으면 전달 없는 백엔드 완료가 롤백됨 |
...
| 알림 | 같은 이벤트가 여러 규칙에 걸려도 수신자별 한 건이며 자신의 알림은 없음 |
...
- 리소스별 요청·응답 DTO와 한국어 enum 표현
- 목록 커서, 필터와 정렬 파라미터
- `version` 충돌 요청·응답
- Markdown과 멘션 직렬화 형식
- multipart 업로드, 파일 메타데이터·다운로드, 프로필 사진과 이슈 파일 연결 DTO
- 25MB 초과, 형식 불일치, 업로드 소유권·워크스페이스 위반과 파일 누락 오류 코드
- 휴지통, 복구와 관계 오류 코드

### 17.2 인증·권한 및 보안 명세서

- 비밀번호 최소 정책과 로그인·메일 요청 속도 제한
- HMAC 토큰 링크 형식과 비밀 키 운영
- 세션 쿠키, CSRF와 출처 검증
- 관리자 권한 검사와 멤버 비활성화 절차

### 17.3 실시간·알림 및 비동기 작업 명세서

- Outbox 이벤트 이름과 최소 payload 스키마
- 알림 수신자 계산과 앵커 규칙
- 워커 잠금, 재시도, 반복 실패와 보존 작업
- 일일 고아 파일·임시 파일·저장소 정합성 작업과 PostgreSQL advisory lock
- PostgreSQL `NOTIFY`와 SSE 이벤트 스키마

## 18. 확정된 데이터 모델 결정

데이터 모델 초안에서 검토한 항목은 다음과 같이 확정했다.

| 순서 | 결정 항목 | 확정 내용 | 상태 |
| ---: | --- | --- | --- |
| Q1 | 멤버 표시 이름 수집 | 회원가입에 표시 이름을 필수로 추가하고 할당·멘션·댓글·활동의 기본 표시값으로 사용 | 확정 |
| Q2 | 멤버 비활성화 시 미완료 담당 작업 | 모든 미완료 작업을 재할당하거나 담당 해제한 뒤에만 비활성화 허용 | 확정 |
| Q3 | 공통 파일 모델 | `files`에 메타데이터를 저장하고 프로필은 `users.avatar_file_id`, 이슈 파일은 `issue_file_attachments`로 연결 | 확정 |
| Q4 | 파일 크기와 저장 방식 | 파일당 최대 25MB, `FILE_STORAGE_ROOT` 로컬 저장, UUID 상대 키 사용 | 확정 |
| Q5 | 고아 파일 정책 | 미연결 24시간·임시 1시간 유예, 워커의 일일 정리와 파일시스템 정합성 경고 | 확정 |
