...
| 기능 이슈 진행률 | 취소된 하위 팀 작업을 제외한 완료 작업 수 ÷ 대상 작업 수 |
...
| 전역 이슈 업무함 | 상위 기능 이슈 상태와 삭제되지 않은 직계 하위 팀 작업의 상태·담당자 조합 |
| 기능 이슈 작업 요약 | 직계 하위 팀 작업, 프로젝트 역할별 팀, 현재 멤버십의 활성 팀 소속과 선행 작업 관계를 페이지 단위로 집계 |
| 전역 이슈 결과 수 | 현재 워크스페이스와 전체 필터를 적용한 기능 이슈 개수 |
...
erDiagram
    ...
    ISSUES ||--o{ ISSUES : "기능 이슈와 하위 작업"
    ISSUES ||--o{ ISSUE_LABELS : "라벨 연결"
    LABELS ||--o{ ISSUE_LABELS : "사용"
    ISSUES ||--o{ ISSUE_BLOCK_RELATIONS : "선행 작업"
    ISSUES ||--o{ ISSUE_BLOCK_RELATIONS : "후행 작업"
    ISSUES ||--o{ COMMENTS : "댓글"
    ISSUES ||--o{ API_HANDOFFS : "작업 전달"
...
| `IssueType` | `FEATURE`, `TEAM_TASK` | 기능 이슈와 팀 작업 |
| `FeatureIssueStatus` | `UNSORTED`, `TODO`, `IN_PROGRESS`, `REVIEW`, `DONE`, `PAUSED`, `CANCELED` | 기능 이슈의 고정 상태. 앞의 네 값은 하위 작업 요약, 뒤의 세 값은 명시적 결정 |
...
| `NotificationType` | `ISSUE_ASSIGNED`, `MENTIONED`, `COMMENT_ADDED`, `ISSUE_COMPLETED`, `ISSUE_CANCELED`, `API_HANDOFF_CREATED`, `API_HANDOFF_FOLLOW_UP_CREATED` | MVP 인앱 알림 유형 |
...
기능 이슈 상태와 시스템 범주의 매핑은 다음과 같이 고정한다.
...
| `next_feature_issue_number` | 다음 기능 이슈 번호, 기본값 1 |
...
팀 작업에만 사용하는 사용자화 가능한 상태다. 기능 이슈는 별도 고정 enum을 사용한다.
...
| `is_default` | 새 팀 작업의 기본 상태. 팀마다 정확히 하나 |
| `version`, `created_at`, `updated_at` | 동시 수정과 시각 |

팀 생성 시 7개 기본 상태를 만들고 `미분류`를 단독 팀 작업의 기본 상태로 지정한다. 사용 중인 상태는 같은 팀의 대체 상태로 모든 이슈를 옮긴 뒤 삭제한다. 기본 상태를 삭제하면 대체 상태에 기본 표시도 함께 이전한다. 시스템 범주는 수정할 수 없고 이름과 순서만 바꿀 수 있다.

기능 이슈의 역할 선택·작업 시작·내가 맡기·최초 전달로 새 하위 팀 작업을 만들 때는 `is_default` 대신 해당 팀의 `UNSTARTED` 범주 중 `position`이 가장 작은 상태를 사용한다. 한 팀이 활성 프로젝트 역할에 연결돼 있는 동안에는 마지막 `UNSTARTED` 상태를 삭제할 수 없으며, 레거시 불일치로 시작 상태를 찾지 못하면 새 작업을 만들지 않고 전체 요청을 롤백한다. 팀 화면에서 직접 만드는 단독 팀 작업은 기존 `is_default`를 계속 사용한다.
...
기능 이슈와 팀 작업은 댓글, 라벨, 검색, 활동, 구독과 관계 기능을 공유하므로 하나의 `issues` 테이블에 `type` 구분자로 저장한다. 유형별로 쓰지 않는 컬럼은 `NULL`이어야 하며 검사 제약과 서버 검증으로 잘못된 조합을 막는다.

| 핵심 컬럼 | 규칙 |
| --- | --- |
| `id`, `workspace_id`, `type` | UUID 기본 키, 워크스페이스와 이슈 유형 |
| `identifier`, `sequence_number` | 불변 표시 ID와 발급 순번 |
| `title` | 필수, 최대 500자 |
| `description_markdown` | 선택 Markdown, 최대 100,000자 |
| `feature_status` | 기능 이슈만 사용 |
| `workflow_state_id` | 팀 작업만 사용 |
| `priority` | 기본값 `NONE` |
| `project_id`, `project_role` | 프로젝트와 팀 작업 역할 |
| `parent_issue_id` | 하위 팀 작업이면 상위 기능 이슈 |
| `team_id`, `assignee_membership_id` | 팀 작업의 고정 팀과 선택 담당자 |
...
| --- | --- | --- |
| `project_id` | 필수 | 선택 |
| `project_role` | `NULL` | 프로젝트가 있으면 필수, 없으면 `NULL` |
| `parent_issue_id` | `NULL` | 선택, 지정하면 같은 프로젝트의 기능 이슈 |
| `team_id` | `NULL` | 필수, 생성 후 변경 불가 |
| `assignee_membership_id` | `NULL` | 선택, 현재 팀의 활성 멤버만 지정 |
| 상태 | `feature_status` 필수 | `workflow_state_id` 필수 |

추가 규칙은 다음과 같다.

- 프로젝트가 있는 팀 작업의 `(project_id, project_role, team_id)`는 `project_role_teams`의 실제 연결과 일치해야 한다.
- 상위 기능 이슈가 있는 팀 작업은 부모와 같은 `project_id`를 사용한다.
- 기능 이슈 아래 팀 작업을 프로젝트에서 개별 제거하거나 다른 프로젝트로 옮길 수 없다.
- 단독 팀 작업은 프로젝트가 없거나, 프로젝트와 역할이 함께 있을 수 있다.
- 담당자 멤버십과 팀 상태는 이슈의 워크스페이스·팀과 일치해야 한다.
- 보관·삭제된 프로젝트, 보관된 팀과 삭제된 상태에는 새 이슈를 연결할 수 없다.

### 8.3 표시 ID 발급

기능 이슈 생성은 워크스페이스 행의 `next_feature_issue_number`, 팀 작업 생성은 팀 행의 `next_issue_number`를 원자적으로 증가시킨다. 증가와 이슈 삽입은 같은 트랜잭션에서 실행한다.

기본 `FEATURE` 생성에서는 프로젝트 역할도 잠그고 요청의 `initialRoles`에 포함된 역할만 팀 카운터 증가와 삽입을 같은 트랜잭션에 포함한다. 생략 또는 빈 배열이면 루트 이슈만 `UNSORTED`로 만들고, 여러 역할을 선택해도 관계 행을 자동 생성하지 않는다. 자동 작업은 새 기능 이슈를 `parent_issue_id`로 연결하고 부모 제목·우선순위·프로젝트를 사용하며 역할 담당 팀의 가장 빠른 `UNSTARTED` 상태, 미할당, 라벨·설명 없음으로 저장한다. 하나 이상의 유효 작업을 만들면 부모 상태를 같은 트랜잭션에서 `TODO`로 계산하며 어느 카운터 증가나 삽입이 실패해도 루트 이슈를 포함한 전체 변경을 롤백한다.

팀 작업 없는 기능 이슈의 `작업 시작`도 부모와 프로젝트 역할을 잠근다. 선택 역할별 기존 미완료 팀 작업은 재사용하고 없는 역할만 생성한다. 완료·취소된 작업만 있는 역할은 새 작업을 만들 수 있으며 복수 역할 중 하나라도 실패하면 팀 카운터 증가와 새 작업을 모두 롤백한다.

기존 `initialRoles`는 담당자 없이 시작하는 계약으로 유지한다. 역할별 담당자를 함께 받는 `roleAssignments`는 같은 요청에서 `initialRoles`와 공존할 수 없고, 담당자가 있으면 역할 팀의 활성 멤버인지 잠금 뒤 검증한다. `requireCurrentUserTeamMembership = true`는 우리 팀에서 시작하는 요청에만 사용하며 서버가 프로젝트 역할에서 팀을 다시 결정한 뒤 현재 멤버십을 확인하고 담당자는 비워 둔다.

- 기능 이슈: `F-{sequence_number}`
- 팀 작업: `{team.key}-{sequence_number}`
- `(workspace_id, identifier)`는 유일하다.
- 기능 이슈 순번은 워크스페이스 안에서, 팀 작업 순번은 팀 안에서 유일하다.
- 삭제·영구 삭제된 번호를 다시 사용하지 않는다.

### 8.4 `issue_labels`

`(issue_id, label_id)`를 복합 기본 키로 사용하고 `workspace_id`를 직접 둔다. 양쪽 리소스가 같은 워크스페이스인지 복합 외래 키로 확인한다. 보관된 라벨의 기존 행은 유지하지만 새 행은 만들 수 없다.

### 8.5 `issue_block_relations`

| 핵심 컬럼 | 규칙 |
| --- | --- |
| `id`, `workspace_id` | UUID 기본 키와 워크스페이스 |
| `blocking_issue_id` | 선행 팀 작업 |
| `blocked_issue_id` | 후행 팀 작업 |
| `created_by_membership_id`, `created_at` | 생성자와 시각 |

`(blocking_issue_id, blocked_issue_id)`는 유일하고 두 ID는 달라야 한다. 양쪽 모두 삭제되지 않은 팀 작업이어야 하며 같은 워크스페이스에 속해야 한다.

전체 순환 여부는 관계 추가 트랜잭션에서 후행 작업부터 도달 가능한 경로를 재귀 조회해 확인한다. MVP 규모에서는 PostgreSQL 재귀 CTE 또는 같은 트랜잭션의 제한 없는 그래프 탐색 한 번으로 충분하며 별도 그래프 저장소나 폐쇄 테이블을 만들지 않는다.

관계는 선행 작업이 완료·취소되어도 삭제하지 않는다. 선행 상태가 `Completed`, `Canceled` 범주이면 외부 응답에서 `resolved = true`로 계산하고 화면에서는 `완료된 선행 작업`, `작업 가능`으로 표시한다.

### 8.6 진행률과 완료 판정

- 팀 작업 완료는 연결된 `workflow_states.category = COMPLETED`로 판단한다.
- 팀 작업 취소는 `workflow_states.category = CANCELED`로 판단한다.
- 기능 이슈 완료·취소는 각각 `feature_status = DONE`, `feature_status = CANCELED`로 판단한다.
- 기능 이슈 진행률은 삭제되지 않은 직계 하위 팀 작업만 사용한다.
- 프로젝트 진행률은 삭제되지 않고 해당 프로젝트에 직접 연결된 팀 작업을 사용한다.
- 취소된 작업은 분자와 분모에서 모두 제외한다.
- 분모가 0이면 계산값은 0%지만 화면은 숫자보다 분석·작업 시작 안내를 우선한다.
- 생성 전 예상 역할은 저장된 팀 작업이 아니므로 진행률에 포함하지 않는다.
- 최초 전달에서 백엔드 완료와 후행 작업 생성·재사용을 한 트랜잭션으로 처리해 중간 100% 진행률을 외부에 노출하지 않는다.

`PAUSED`, `CANCELED`, `DONE`이 아닌 기능 이슈의 `feature_status`는 삭제되지 않고 `CANCELED` 범주가 아닌 직계 하위 팀 작업으로 다음 순서에 따라 계산해 저장한다.

1. 유효 작업이 없으면 `UNSORTED`다.
2. 유효 작업이 하나 이상이고 `STARTED`·`COMPLETED` 작업이 하나도 없으면 `TODO`다. 팀별 `BACKLOG` 상태도 이미 실행 팀이 정해진 하위 작업이므로 부모는 `TODO`다.
3. `STARTED` 또는 `COMPLETED` 작업이 하나 이상 있고 미완료 작업이 남아 있으면 `IN_PROGRESS`다.
4. 유효 작업이 하나 이상이고 모두 `COMPLETED`면 `REVIEW`다.

팀 작업 생성·상태 변경·삭제·복구와 부모 아래 작업 재사용은 부모 기능 이슈를 잠근 뒤 상태 계산에 필요한 직계 작업을 다시 읽는다. 부모 상태 변경, 팀 작업 변경, 활동과 Outbox는 같은 트랜잭션에 포함한다. 계산 결과가 이전 값과 같으면 부모 버전과 상태 활동을 추가로 증가시키지 않는다.

`PAUSED`, `CANCELED`, `DONE`에서는 자동 계산을 중지한다. `RESUME`은 `PAUSED`, `REOPEN`은 `CANCELED` 또는 `DONE`을 현재 직계 작업 기준 계산값으로 되돌린다. `COMPLETE`는 계산 결과가 `REVIEW`인 경우에만 `DONE`으로 전환한다. 이 상태에서 기존 하위 작업의 이력·속성 변경은 부모 상태를 바꾸지 않지만 새 하위 작업을 추가하거나 완료·취소된 역할 작업을 다시 시작하려면 먼저 부모를 다시 열거나 재개해야 한다.

### 8.7 전역 이슈 업무함과 작업 요약

전역 목록은 삭제되지 않은 `FEATURE`만 대상으로 하며 다음 조건을 저장하지 않고 조회 시 계산한다.

| 업무함 | 계산 조건 |
| --- | --- |
| `REVIEW_REQUIRED` | 상위 기능 이슈 상태가 `UNSORTED` |
| `ASSIGNMENT_REQUIRED` | 완료·취소가 아닌 하위 팀 작업 중 담당자가 없는 작업이 하나 이상 있음 |
| `IN_PROGRESS` | 상위 기능 이슈 상태가 `IN_PROGRESS` |
| `COMPLETION_REQUIRED` | 상위 기능 이슈 상태가 `REVIEW` |
| `COMPLETED` | 상위 기능 이슈 상태가 `DONE` |

유효 팀 작업이 모두 취소되면 상위 이슈는 `UNSORTED`로 계산돼 `REVIEW_REQUIRED`에 다시 포함된다. `ASSIGNMENT_REQUIRED`와 `IN_PROGRESS`는 동시에 참일 수 있다.

기능 이슈 목록의 `workflowSummary`는 다음 계산값을 제공한다.

- 삭제되지 않은 전체 팀 작업 수, 완료 수, 취소 수와 완료·취소가 아닌 미할당 수
- 현재 미완료 역할과 구체적인 선행 작업 대기 대상
- 취소 제외 대상 작업 존재 여부와 모든 대상 작업 완료 여부
- 현재 역할과 프로젝트 역할 팀을 묶은 `activeRoleTeams`
- 현재 사용자가 활성 멤버인 프로젝트 역할의 `currentUserTeamRoles`
- 현재 사용자에게 할당된 미완료 하위 작업의 ID·표시 ID·역할을 담은 `currentUserAssignedTeamTasks`

목록 항목의 `createdBy`와 페이지 응답의 `totalCount` 역시 기존 생성자 관계와 전체 필터에서 계산한다. 이 값들을 위해 `FEATURE`에 팀·담당자·요약 컬럼이나 별도 집계 모델을 추가하지 않는다. 목록 페이지의 기능 이슈 ID 집합을 먼저 확정한 뒤 하위 작업·프로젝트 역할·팀 멤버십·선행 관계를 고정된 수의 일괄 쿼리로 읽어 행별 추가 조회와 N+1을 만들지 않는다.
...
댓글 삭제는 30일 휴지통 대상이 아니다. 삭제 시 `deleted_at`을 기록하고 본문을 제거해 복원하지 않으며, 행과 작성자·시각은 타임라인 순서 유지를 위해 이슈 영구 삭제 전까지 남긴다.
...
`(issue_id, membership_id)`를 복합 기본 키로 사용한다. 이슈 생성자, 현재 또는 과거 담당자와 멘션 대상은 자동 구독된다. 담당 해제나 멘션 본문 수정으로 자동 구독을 제거하지 않는다. MVP에는 수동 구독 해제 화면이 없으므로 구독 이유별 행이나 알림 환경설정 테이블을 만들지 않는다.

### 9.4 `api_handoffs`

| 핵심 컬럼 | 규칙 |
| --- | --- |
| `id`, `workspace_id`, `issue_id` | UUID 기본 키와 백엔드 역할 팀 작업 |
| `kind` | `INITIAL` 또는 `FOLLOW_UP` |
| `sequence_number` | 이슈 안에서 1부터 증가, 유일 |
| `body_markdown` | 템플릿 기반 본문, 최대 50,000자 |
| `author_membership_id`, `created_at` | 작성자와 시각 |

작업 전달은 수정·삭제하지 않는 추가 전용 데이터다. 이슈별 `INITIAL`은 최대 한 건이고 `FOLLOW_UP`은 최초 전달 뒤에만 추가한다. API 명세 링크와 변경 요약 등 템플릿 제목만 있고 실질 내용이 없는 본문은 유효한 전달로 인정하지 않는다. 작업 전달 본문의 멘션 참조는 저장하지 않고 `MARKDOWN_INVALID`로 거부하며, 전달별 멘션 관계나 별도 `MENTIONED` 수신자는 두지 않는다.

프로젝트에 웹 또는 앱 역할이 설정된 백엔드 역할 팀 작업을 완료할 때는 후행 작업의 현재 존재 여부와 무관하게 유효한 최초 전달을 요구한다. 완료 요청의 `destinationRoles`는 저장 컬럼이 아니라 트랜잭션 입력이며 프로젝트에 설정된 프론트 역할의 비어 있지 않은 부분집합이어야 한다. 백엔드 전용 프로젝트만 최초 전달 없이 완료할 수 있다.

최초 전달 트랜잭션은 백엔드 작업과 부모 이슈, 대상 프로젝트 역할을 잠그고 역할별 삭제되지 않은 하위 팀 작업을 조회한다. 미완료 작업이 한 건 이상이면 모두 재사용하고, 기존 작업이 없으면 역할 담당 팀의 가장 빠른 `UNSTARTED` 상태로 미할당 작업 한 건을 생성한다. 완료·취소된 작업만 있으면 새 작업을 만들지 않고 범위 확인 충돌로 중단하며, 기존 작업의 프로젝트·역할·팀이 현재 설정과 다르면 자동 수정하지 않는다. 이번 전달에서 새로 생성한 후행 작업에만 백엔드 작업과의 차단 관계를 추가한다. 기존 후행 작업은 이슈 생성부터 병렬로 진행했을 수 있으므로 기존 사용자 관계를 유지하고 새 관계를 추론하지 않는다.

전달 생성, 백엔드 상태 변경, 후행 작업 생성·재사용, 차단 관계, 부모 진행률 조회에 필요한 상태, 활동과 Outbox 발행은 함께 커밋한다. 이미 처리된 동일 최초 완료 요청은 새 전달·작업·관계·이벤트를 만들지 않고 `ISSUE_VERSION_CONFLICT`를 반환하며, 웹은 최신 이슈를 조회해 완료된 전달과 후행 작업을 성공 결과로 표시한다. `FOLLOW_UP`은 같은 부모와 프로젝트의 기존 미완료 웹·앱 작업을 참조하되 새 팀 작업이나 차단 관계를 자동 생성하지 않는다.

상위 기능 이슈와 전달받은 프론트 작업은 새 전달 행을 만들지 않고 기존 `api_handoffs` 원본을 조회한다. 최초 전달 대상은 `BACKEND_WORK_DELIVERED` 활동의 후행 작업 ID와 기존 차단 관계로 연결하고, 추가 전달 활동에는 대상 후행 작업 ID를 함께 기록한다. 기존 추가 전달처럼 대상 ID가 없는 이력은 같은 원본 백엔드 전달 흐름과 부모·프로젝트의 기존 프론트 작업을 이용해 호환 조회한다. 변경 요약은 `body_markdown`의 고정 섹션에서 계산하며 별도 컬럼에 복제하지 않는다.
...
일반 이슈 첨부와 설명·댓글·작업 전달의 인라인 이미지를 같은 이슈 권한과 수명주기로 관리한다.
...
업무 변경과 Outbox 삽입을 같은 트랜잭션에 둔다. 최초·추가 전달 이벤트에는 실제 후행 작업 ID와 사건 시점에 중복 제거한 알림 후보 멤버십 ID를 최소 payload로 저장한다. 워커는 `FOR UPDATE SKIP LOCKED`로 처리할 행을 가져가고 제한된 지수형 재시도를 사용한다. `last_error_code`에는 정제된 코드만 저장하고 요청·응답 본문과 비밀 값을 넣지 않는다.
...
`(event_id, recipient_membership_id)`를 유일하게 해 멘션·구독·담당·전달 규칙이 겹치거나 Outbox가 재처리돼도 수신자별 한 건만 만든다. 최초 전달에서 재사용한 작업은 현재 담당자와 자동 구독자, 새 미할당 작업은 대상 팀의 사건 당시 활성 멤버를 후보로 삼는다. 추가 전달은 기존 후행 작업의 담당자와 자동 구독자만 사용한다. 행위자와 수신자가 같거나 처리 시점의 수신 멤버십이 비활성이면 생성하지 않는다.

전달 알림의 `issue_id`는 원본 데이터 소유자가 아니라 이동 대상을 나타낸다. 수신자의 담당·구독·활성 팀 멤버 조건과 맞는 후행 작업을 우선하고, 없으면 원본 백엔드의 상위 이슈, 상위도 없으면 원본 백엔드 작업을 사용한다. `handoff_id`는 어느 이동 대상을 사용해도 같은 원본 전달을 가리킨다. 알림 본문 스냅샷은 저장하지 않고 유형과 현재 이슈 정보로 한국어 요약을 만든다.
...
| 표시 ID 유일 | `issues(workspace_id, identifier)` 유일 제약 |
| 발급 순번 유일 | 기능 이슈와 팀 작업 유형별 부분 유일 인덱스 |
| 프로젝트 역할 중복 금지 | `project_role_teams(project_id, role)` 복합 기본 키 |
| 이슈 유형별 필수·금지 필드 | `IssueType`별 `CHECK` 제약 |
| 프로젝트 날짜 순서 | 두 날짜가 있으면 `target_date >= start_date` 검사 제약 |
| 중복·자기 차단 금지 | 관계 쌍 유일 제약과 두 ID가 다르다는 검사 제약 |
| 최초 전달 중복 금지 | `kind = INITIAL`인 이슈별 부분 유일 인덱스 |
...
- 내가 맡기의 후보 작업이 같은 부모·역할·팀의 미할당 미완료 작업인지 확인하고 다른 담당자가 생긴 경우 변경하지 않음
- 일괄 배정의 모든 작업 ID·버전·부모·역할·팀·현재 담당자와 후보 활성 팀 멤버를 변경 전 전부 확인
- 차단 관계의 양쪽 유형·워크스페이스 일치와 전체 순환 부재
- 프론트 역할이 있는 프로젝트의 백엔드 작업 완료 시 대상 역할이 있는 최초 전달 존재
- 최초 전달의 후행 작업 다건 재사용, 완료·취소 범위 충돌과 현재 프로젝트 역할·팀 일치
- 업로드 파일의 실제 크기·형식, 업로더와 파일 범위 검증
- 프로필 사진 파일의 사용자 소유·`USER_PROFILE` 범위 검증
- 이슈 파일의 `WORKSPACE` 범위, 워크스페이스·이슈·댓글·작업 전달 일치 검증
...
| 기본 이슈 생성 | 프로젝트 역할·시작 상태 잠금, 루트 이슈와 선택한 `initialRoles`의 역할별 카운터 증가·`UNSTARTED` 팀 작업, 부모 자동 상태, 라벨, 구독, 활동, Outbox. 빈 선택은 팀 작업 없는 `UNSORTED` |
| 이슈 작업 시작 | 부모·프로젝트 역할·시작 상태 잠금, `initialRoles` 또는 역할별 선택 담당자가 있는 `roleAssignments` 검증, 선택 역할의 기존 미완료 작업 재사용, 없는 역할의 카운터 증가·`UNSTARTED` 팀 작업, 부모 자동 상태, 담당자·구독, 활동, 알림 후보·Outbox. `requireCurrentUserTeamMembership`이면 현재 역할 팀 멤버십도 잠금·재검증 |
| 내가 맡기 | 부모·역할 팀·현재 멤버십과 후보 작업 잠금, 후보 0건이면 새 작업 생성·1건이면 재사용·여러 건이면 선택 요구, 미할당 재검증, 자신에게 할당, 구독, 활동, 자기 알림을 제외한 Outbox |
| 팀 작업 일괄 배정 | 부모와 모든 대상 팀 작업·버전·역할 팀·후보 멤버 잠금, 전체 검증 뒤 담당자·구독·활동·알림 후보·Outbox 일괄 반영. 한 건 충돌 시 전체 롤백 |
| 수동 팀 작업 생성 | 팀 카운터 증가, 팀 작업, 라벨, 구독, 활동, Outbox |
| 이슈 생성·설명 저장 | 이슈·설명, 파일 소유·범위 검증, 일반 첨부·설명 이미지 연결과 `unlinked_at`, 멘션, 활동, Outbox |
| 이슈 속성 변경 | 예상 버전 확인, 일반 속성 또는 허용된 기능 상태 행동, 현재 직계 팀 작업 재계산, 활동, Outbox, 버전 증가 |
| 상태 삭제·대체 | 대상 이슈 상태 일괄 교체, 활동, 기본 상태 이전, 기존 상태 삭제, Outbox |
| 프로젝트 역할 변경 | 연결 작업 존재 확인, 역할별 팀 변경, 프로젝트 활동·버전, Outbox |
| 작업 순서 관계 변경 | 양쪽 잠금, 순환 확인, 내부 차단 관계, 양쪽 활동·버전, Outbox |
| 댓글 저장 | 댓글, 멘션 집합, 댓글 이미지 연결과 `unlinked_at`, 자동 구독, 활동, Outbox |
| 백엔드 작업 전달·완료 | 부모·백엔드·대상 역할·시작 상태 잠금, 최초 전달 검증·생성, 전달 이미지 연결과 `unlinked_at`, 상태 변경, `UNSTARTED` 후행 작업 생성 또는 다건 재사용, 새 후행 작업의 관계, 부모 진행률·자동 상태, 활동, 알림 후보 스냅샷과 Outbox |
| 추가 전달 | 추가 전용 전달, 기존 후행 작업 확인, 전달 이미지 연결과 `unlinked_at`, 활동, 담당자·구독자 후보 스냅샷과 Outbox. 팀 작업 생성 없음 |
...
- 기능 이슈는 삭제되지 않은 하위 팀 작업이 있으면 삭제할 수 없다.
- 팀 작업은 삭제되지 않은 다른 팀 작업을 차단 중이면 삭제할 수 없다.
- 삭제 시 `purge_at = deleted_at + 30일`을 기록한다.
- 일반 목록, 검색, 진행률, 차단 계산과 직접 조회는 삭제된 이슈를 제외한다.
- 댓글, 라벨, 구독, 멘션, 차단 관계, 작업 전달, 활동, 파일 연결과 알림 앵커는 보존 기간 동안 유지한다.
...
| 조회 | 권장 인덱스 |
| --- | --- |
| 정확한 표시 ID 검색 | `issues(workspace_id, identifier)` 유일 B-tree |
| 제목 부분 검색 | `workspace_id`, 휴지통 조건과 `lower(title)` 검색. 필요 시 `pg_trgm` GIN 인덱스 |
| 팀 이슈 목록 | `issues(workspace_id, team_id, deleted_at, updated_at, id)` |
| 내 작업 | `issues(workspace_id, assignee_membership_id, deleted_at, updated_at, id)` |
| 전역 기능 이슈 | `issues(workspace_id, type, deleted_at, updated_at, id)`와 현재 페이지 부모 ID의 하위 작업·역할·담당자 일괄 조회 |
| 프로젝트 계층 | `issues(workspace_id, project_id, parent_issue_id, deleted_at)` |
...
- enum 값 변경, 컬럼 삭제와 강한 `NOT NULL` 추가는 기존·신규 API와 워커가 잠시 공존할 수 있는 확장 후 축소 순서로 적용한다.
...
| 표시 ID | 같은 워크스페이스·팀의 동시 이슈 생성에도 번호와 표시 ID가 중복되지 않음 |
| 이슈 유형 | 기능 이슈에 팀·담당자가 들어가거나 팀 작업에 잘못된 상태가 들어가지 않음 |
| 최초 작업 생성 | 생략·빈 선택은 루트만, 단일·복수 `initialRoles`는 선택 역할 작업만 루트와 함께 생성되고 일부 실패 시 모두 롤백됨 |
| 작업 시작 | 팀 작업 없는 이슈에서 단일·복수 역할을 시작하고 기존 미완료 역할 작업을 중복 생성하지 않으며 일부 실패 시 모두 롤백됨 |
| 기능 이슈 자동 상태 | 작업 없음·시작 전만·일부 시작 또는 완료·전체 완료·전체 취소 조합이 각각 `UNSORTED`, `TODO`, `IN_PROGRESS`, `REVIEW`, `UNSORTED`로 저장되고 팀 작업 변경과 부모 상태가 함께 커밋됨 |
| 기능 이슈 상태 행동 | `PAUSED`, `CANCELED`, `DONE`에서 자동 전이가 멈추고 재개·다시 열기는 현재 팀 작업으로 재계산되며 `REVIEW`가 아닌 이슈 완료와 닫힌 이슈의 작업 시작이 거부됨 |
| 역할별 담당자 시작 | `initialRoles` 하위 호환, `roleAssignments` 상호 배타, 역할 팀 활성 멤버 검증과 복수 역할 전체 커밋·롤백 |
...
| 일괄 배정 | 여러 역할 작업의 부모·버전·후보를 모두 검증하고 한 건 실패 시 담당자·활동·구독·Outbox가 전부 롤백됨 |
| 프로젝트 역할 | 역할과 담당 팀이 다른 팀 작업을 프로젝트에 연결할 수 없음 |
| 상하위 관계 | 하위 작업이 부모와 다른 프로젝트를 가질 수 없음 |
| 차단 관계 | 자기 차단, 중복과 길이가 2 이상인 순환을 모두 거부함 |
| 작업 전달 | 프로젝트에 프론트 역할이 있으면 전달 없는 백엔드 완료가 롤백되고 최초 전달 시 후행 작업 생성·다건 재사용·새 후행 관계·부모 진행률이 원자적으로 반영됨. 기존 병렬 작업에는 관계를 추가하지 않음 |
| 전달 조회 | 상위 이슈와 전달받은 프론트 작업이 같은 `ApiHandoff` 원본과 대상 관계를 조회함 |
| 전달 재시도 | 같은 완료 요청 재시도에 최초 전달·후행 작업·관계·Outbox가 중복되지 않고 추가 전달은 팀 작업을 만들지 않음 |
| 파일 제한 | 프론트 최적화 여부와 무관하게 25MB 초과, 빈 파일과 허용되지 않은 인라인 형식이 서버에서 거부됨 |
| 파일 격리 | 미연결 파일은 업로더만, 연결 파일은 같은 워크스페이스의 이슈 접근자만 조회·다운로드함 |
| 파일 연결 | Markdown 파일 ID와 설명·댓글·전달 연결이 일치하고 다른 이슈의 파일 재사용이 거부됨 |
...
| 알림 | 새 미할당 후행 작업은 대상 팀 활성 멤버, 재사용 작업은 담당자·자동 구독자를 사용하고 같은 이벤트 재처리에도 수신자별 한 건이며 작성자 자신의 알림은 없음 |
