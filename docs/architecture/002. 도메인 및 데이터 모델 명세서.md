...
미완료 웹 또는 앱 역할 팀 작업을 차단하는 백엔드 역할 팀 작업을 완료할 때는 유효한 최초 전달의 존재를 같은 트랜잭션에서 확인한다. 전달 생성, 백엔드 상태 변경, 활동과 Outbox 발행은 함께 커밋하며, 전달 알림 후보는 실제 미완료 후행 작업의 담당자와 구독자로 계산한다.
