...
- B3는 실데이터와 구버전 프로세스가 존재할 수 있으므로 expand/backfill/contract로 배포한다. expand 단계에서 `project_teams`, `team_works.project_team_id`, `issue_templates.initial_project_team_id`를 추가하고 기존 프로젝트·팀 조합과 TeamWork·템플릿을 검증하며 backfill한다.
