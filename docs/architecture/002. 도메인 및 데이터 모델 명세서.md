...
전달 트랜잭션은 원본 팀 작업과 이슈, 대상 참여 관계를 잠그고 팀별 삭제되지 않은 작업을 조회한다. 미완료 작업이 있으면 재사용하고 없으면 대상 팀의 기본 비종료 워크플로 상태로 미할당 작업을 생성한다. 기존 작업의 프로젝트·참여 관계·팀이 다르면 자동 수정하지 않는다. 전달 대상은 모두 `api_handoff_targets`에 명시한다.
...
- 활성 프로젝트 참여 팀이 시작 가능한 기본 워크플로 상태를 유지하는지 확인
