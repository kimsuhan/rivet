...
개인 저장된 보기는 소유 membership과 workspace에만 속하며 팀 공유나 구독 관계를 만들지 않는다. `resource_type`은 `ISSUES`, `MY_WORK`만 허용하고, `configuration`에는 서버 허용 목록의 검색·필터·정렬·표시 옵션만 정규화된 JSON으로 저장한다. 임의 URL, SQL, 미지원 속성과 중첩 OR 구조는 저장하지 않는다.
