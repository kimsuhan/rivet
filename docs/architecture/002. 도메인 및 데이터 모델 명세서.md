...
| `FeatureIssueStatus` | `UNSORTED`, `TODO`, `IN_PROGRESS`, `REVIEW`, `DONE`, `PAUSED`, `CANCELED` | 기능 이슈의 고정 한국어 상태 |
...
팀 생성 시 7개 기본 상태를 만들고 `미분류`를 기본 상태로 지정한다. 사용 중인 상태는 같은 팀의 대체 상태로 모든 이슈를 옮긴 뒤 삭제한다. 기본 상태를 삭제하면 대체 상태에 기본 표시도 함께 이전한다. 시스템 범주는 수정할 수 없고 이름과 순서만 바꿀 수 있다.
...
기본 `FEATURE` 생성에서는 프로젝트 역할도 잠그고 요청의 `initialRoles`에 포함된 역할만 팀 카운터 증가와 삽입을 같은 트랜잭션에 포함한다. 생략 또는 빈 배열이면 루트 이슈만 만들고, 여러 역할을 선택해도 관계 행을 자동 생성하지 않는다. 자동 작업은 새 기능 이슈를 `parent_issue_id`로 연결하고 부모 제목·우선순위·프로젝트를 사용하며 역할 담당 팀의 기본 상태, 미할당, 라벨·설명 없음으로 저장한다. 어느 카운터 증가나 삽입이 실패해도 루트 이슈를 포함한 전체 변경을 롤백한다.
...
- 기능 이슈 완료·취소는 고정 상태와 시스템 범주의 매핑으로 판단한다.
...
| 업무함 | 계산 조건 |
| --- | --- |
| `REVIEW_REQUIRED` | 상위 이슈가 완료·취소가 아니고 삭제되지 않은 하위 팀 작업이 한 건도 없음 |
| `ASSIGNMENT_REQUIRED` | 완료·취소가 아닌 하위 팀 작업 중 담당자가 없는 작업이 하나 이상 있음 |
| `IN_PROGRESS` | 완료·취소가 아닌 하위 팀 작업이 하나 이상 있음 |
| `COMPLETION_REQUIRED` | 취소를 제외한 하위 팀 작업이 하나 이상이고 모두 완료됐으며 상위 이슈는 완료·취소가 아님 |
| `COMPLETED` | 상위 기능 이슈 상태가 `DONE` |

완료·취소된 기존 팀 작업만 있는 이슈는 삭제되지 않은 하위 작업이 존재하므로 `REVIEW_REQUIRED`로 되돌리지 않는다. `ASSIGNMENT_REQUIRED`와 `IN_PROGRESS`는 동시에 참일 수 있다.
...
최초 전달 트랜잭션은 백엔드 작업과 부모 이슈, 대상 프로젝트 역할을 잠그고 역할별 삭제되지 않은 하위 팀 작업을 조회한다. 미완료 작업이 한 건 이상이면 모두 재사용하고, 기존 작업이 없으면 역할 담당 팀의 기본 상태로 미할당 작업 한 건을 생성한다. 완료·취소된 작업만 있으면 새 작업을 만들지 않고 범위 확인 충돌로 중단하며, 기존 작업의 프로젝트·역할·팀이 현재 설정과 다르면 자동 수정하지 않는다. 이번 전달에서 새로 생성한 후행 작업에만 백엔드 작업과의 차단 관계를 추가한다. 기존 후행 작업은 이슈 생성부터 병렬로 진행했을 수 있으므로 기존 사용자 관계를 유지하고 새 관계를 추론하지 않는다.
...
| 기본 이슈 생성 | 프로젝트 역할 잠금, 루트 이슈와 선택한 `initialRoles`의 역할별 카운터 증가·팀 작업, 라벨, 구독, 활동, Outbox. 빈 선택은 팀 작업 없음 |
| 이슈 작업 시작 | 부모·프로젝트 역할 잠금, `initialRoles` 또는 역할별 선택 담당자가 있는 `roleAssignments` 검증, 선택 역할의 기존 미완료 작업 재사용, 없는 역할의 카운터 증가·팀 작업, 담당자·구독, 활동, 알림 후보·Outbox. `requireCurrentUserTeamMembership`이면 현재 역할 팀 멤버십도 잠금·재검증 |
...
| 이슈 속성 변경 | 예상 버전 확인, 이슈·관계, 활동, Outbox, 버전 증가 |
...
| 백엔드 작업 전달·완료 | 부모·백엔드·대상 역할 잠금, 최초 전달 검증·생성, 전달 이미지 연결과 `unlinked_at`, 상태 변경, 후행 작업 생성 또는 다건 재사용, 새 후행 작업의 관계, 부모 진행률 상태, 활동, 알림 후보 스냅샷과 Outbox |
