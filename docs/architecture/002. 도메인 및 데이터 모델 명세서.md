...
팀 생성 시 `Backlog`, `Unstarted`, `Started`, `Completed`, `Canceled` 범주 순서로 7개 기본 상태를 만들고 `미분류`를 기본값으로 지정한다. 관리자가 상태를 추가하면 선택한 범주의 마지막 `position`에 삽입하고 이후 범주의 상태는 충돌 없는 역순 갱신으로 한 칸씩 이동한다. 시스템 범주는 생성 후 수정할 수 없고 순서는 같은 범주 안에서만 바꿀 수 있다. 전체 순서 요청이 범주 순서를 깨면 서버가 거절한다. 모든 범주의 상태를 기본값으로 지정할 수 있으며 변경 시 기존 팀 작업은 이동하지 않는다. 사용 중인 상태는 같은 팀의 대체 상태로 모든 팀 작업을 옮긴 뒤 삭제한다. 기본 상태를 삭제하면 대체 상태에 기본 표시도 함께 이전한다.
