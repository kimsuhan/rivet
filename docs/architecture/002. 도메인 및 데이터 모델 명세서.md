...
- Outbox, 알림, 이메일 발송 기록과 CSV 내보내기·가져오기 기록
...
| 프로젝트 팀 참여 활성 여부 | `project_teams.is_active`와 팀 보관 상태 |
| 전역 이슈 업무함 | 이슈 상태와 삭제되지 않은 팀 작업의 상태·담당자 조합 |
| 이슈 작업 요약 | 이슈의 팀 작업, 프로젝트 참여 팀과 현재 멤버십의 활성 팀 소속을 페이지 단위로 집계 |
...
erDiagram
    ...
    PROJECTS ||--o{ PROJECT_TEAMS : "참여 팀"
    TEAMS ||--o{ PROJECT_TEAMS : "참여"
...
| `ProjectRole` | `BACKEND`, `WEB_FRONTEND`, `APP_FRONTEND` | B3 expand 기간의 구버전 쓰기 호환값. 신규 제품 계약과 조회 응답에는 노출하지 않으며 contract migration에서 제거한다. |
...
| `ImportRunStatus` | `PREVIEWED`, `VALIDATION_FAILED`, `VALIDATED`, `PROCESSING`, `SUCCEEDED`, `FAILED` | CSV 가져오기 실행 상태 |
...
한국어 표시는 `UNSORTED` 접수됨, `TODO` 할 일, `IN_PROGRESS` 진행 중, `REVIEW` 배포 대기, `DONE` 완료, `PAUSED` 일시 중지, `CANCELED` 취소로 고정한다. enum과 시스템 범주 매핑은 유지하고 상태 전이 책임만 서버 계산으로 변경한다.
...
### 5.3.1 `workspace_invitation_continuations`

초대 링크를 가입·인증·로그인 사이에서 안전하게 이어 주는 단기 서버 상태다.

| 핵심 컬럼 | 규칙 |
| --- | --- |
| `id`, `one_time_token_id` | UUID 기본 키, 원본 워크스페이스 초대 토큰 외래 키 |
| `user_id` | 가입 또는 로그인 뒤 귀속한 사용자. 귀속 전에는 `NULL` |
| `token_hash` | HttpOnly 쿠키에 담긴 무작위 진행 값의 해시, 유일 |
| `consumed_at`, `revoked_at` | 수락 완료와 사용자가 초대를 닫은 시각 |
| `created_at`, `updated_at` | 생성과 변경 시각 |

원문 진행 값은 데이터베이스에 저장하지 않는다. 사용자별 활성 진행 상태는 하나만 허용하고 새 상태를 귀속할 때 이전 상태를 폐기한다. 원본 `one_time_tokens`가 보존 기간 뒤 제거되면 연결된 진행 상태도 함께 제거한다.
...
| `role` | 팀 안의 역할 `MEMBER` 또는 `LEAD`, 기본값 `MEMBER` |
...
한 팀은 여러 `LEAD`를 둘 수 있고 기존 팀은 팀장이 없는 상태도 허용한다. 팀장 지정과 해제는 워크스페이스 관리자만 수행한다. 제거된 팀 멤버는 `LEAD`일 수 없으므로 팀장을 팀에서 제외하려면 먼저 역할을 `MEMBER`로 되돌린다. 팀장은 지정된 활성 팀의 이름·설명, 멤버, 팀 초대와 워크플로만 관리하며 팀 키·보관·프로젝트 참여 구성은 바꾸지 못한다.
...
| `is_default` | 담당자 없는 새 작업과 새 전달 대상 작업의 초기 상태. 팀마다 정확히 하나 |
| `disabled_at` | 신규 선택에서 제외된 시각. 사용 중이면 `NULL` |
| `version`, `created_at`, `updated_at` | 동시 수정과 시각 |

팀 생성 시 `Backlog`, `Unstarted`, `Started`, `Completed`, `Canceled` 범주 순서로 7개 기본 상태를 만들고 `미분류`를 기본값으로 지정한다. 관리자 또는 해당 팀장이 상태를 추가하면 선택한 범주의 마지막 `position`에 삽입하고 이후 범주의 상태는 충돌 없는 역순 갱신으로 한 칸씩 이동한다. 시스템 범주는 생성 후 수정할 수 없고 순서는 같은 범주 안에서만 바꿀 수 있다. 전체 순서 요청이 범주 순서를 깨면 서버가 거절한다. 모든 범주의 활성 상태를 기본값으로 지정할 수 있으며 변경 시 기존 팀 작업은 이동하지 않는다. 상태 사용을 중지하면 기존 팀 작업 연결과 진행률 집계에는 남지만 신규 선택·기본값·CSV 가져오기 대상에서는 제외한다. 기본 상태와 활성 프로젝트 팀의 마지막 활성 `UNSTARTED` 상태는 먼저 대체 조건을 충족해야 사용 중지할 수 있다. 사용 중인 상태는 같은 팀의 활성 대체 상태로 모든 팀 작업을 옮긴 뒤 삭제한다. 기본 상태를 삭제하면 대체 상태에 기본 표시도 함께 이전한다.

담당자 없이 새 팀 작업을 만들거나 전달 대상 팀에 재사용할 미완료 작업이 없으면 해당 팀의 `is_default` 상태를 사용한다. 담당자를 지정해 새 팀 작업을 만들거나 담당자 없는 작업을 처음 배정할 때는 해당 팀의 `UNSTARTED` 범주 중 `position`이 가장 작은 상태를 사용한다. 한 팀이 활성 프로젝트 역할에 연결돼 있는 동안에는 마지막 `UNSTARTED` 상태를 삭제할 수 없으며 필요한 상태를 찾지 못하면 새 작업을 만들지 않고 전체 요청을 롤백한다.

### 6.4 `workspace_invitation_teams`

하나의 대기 중인 워크스페이스 초대를 한 개 이상의 팀과 연결한다. `(invitation_id, team_id)`가 기본 키이며 `workspace_id`, 초대한 팀 관리자 멤버십과 생성 시각을 함께 저장한다. 초대를 수락하면 보관되지 않은 대상 팀마다 `team_members`를 생성하거나 복귀시키고 역할은 `MEMBER`로 둔다. 같은 이메일의 기존 대기 초대에 다른 팀을 추가할 때 새 이메일을 발송하지 않고 팀 대상만 추가한다.
...
| `logo_file_id` | 선택, 같은 워크스페이스 `files` 한 건과 1:1 연결 |
| `status` | 기본값 `PLANNED` |
...
프로젝트 로고는 `WORKSPACE` 범위의 JPG·PNG·WebP 파일만 사용한다. 프로젝트 생성·교체 시 새 파일의 `unlinked_at`을 `NULL`로 바꾸고 제거·교체된 파일은 현재 시각으로 되돌린다. 보관과 휴지통 기간에는 연결을 유지하며 프로젝트 영구 삭제 시 연결을 해제해 24시간 파일 정리 유예를 시작한다.

### 7.2 `project_teams`

| 핵심 컬럼 | 규칙 |
| --- | --- |
| `id`, `workspace_id`, `project_id`, `team_id` | UUID 기본 키와 동일 워크스페이스의 프로젝트·실제 팀. `(project_id, team_id)` 유일 |
| `is_active`, `deactivated_at` | 현재 참여 여부와 참여 종료 시각 |
| `deployment_tracking_enabled` | 이 프로젝트에서 해당 팀의 운영 배포 현황을 관리하는지 여부. 기본값 `false` |
| `created_at`, `updated_at` | 생성·수정 시각 |

프로젝트는 참여 팀 없이 생성할 수 있다. TeamWork 시작과 전달 대상 선택은 보관되지 않은 활성 `ProjectTeam`만 사용한다. 같은 팀을 다시 추가하면 새 행을 만들지 않고 기존 행을 활성화해 과거 작업·전달·활동·알림 연결을 보존한다.

미완료 TeamWork가 있는 참여 팀 제외는 서버가 거부한다. 완료·취소 이력만 있으면 행을 삭제하지 않고 비활성화한다. 워크스페이스 관리자와 현재 프로젝트 리드만 참여 구성을 바꾸며 팀장 등 다른 권한은 팀 멤버·워크플로 관리 범위를 넓히지 않는다.
...
| `project_team_id`, `team_id` | 해당 프로젝트의 참여 관계와 실제 담당 팀 |
| `workflow_state_id` | 같은 팀의 현재 워크플로 상태 |
| `assignee_membership_id` | 선택 담당자, 현재 팀의 활성 멤버만 허용 |
| `work_note_markdown` | 선택 Markdown, 최대 10,000자 |
| `deployment_status` | `NOT_APPLICABLE`, `PENDING`, `DEPLOYED`, `REDEPLOY_REQUIRED`. 기존 행 기본값은 `NOT_APPLICABLE` |
| `deployment_group_id` | 선택, 같은 이슈에서 함께 배포할 그룹 |
| `deployed_at`, `deployed_by_membership_id` | 마지막 운영 배포 완료 시각과 기록한 멤버십 |
| `created_by_membership_id` | 작업을 시작하거나 생성한 멤버십 |
| `deleted_at` | 이슈 안에서 실행 범위를 제거한 시각, 활성 작업이면 `NULL` |
| `version`, `created_at`, `updated_at` | 동시 수정과 정렬 시각 |

`work_note_markdown`은 이슈 전체 설명을 반복하지 않고 해당 팀의 구현 범위·조사 결과·주의사항만 적는다. Markdown의 제목, 목록, 인용, 링크, 코드 블록과 활성 워크스페이스 멤버 멘션을 지원하고 인라인 이미지와 파일 연결은 지원하지 않는다. 해당 팀의 활성 멤버만 수정한다. 팀 작업에는 제목, 설명, 우선순위, 라벨, 프로젝트, 일반 첨부파일, 독립 댓글과 독립 휴지통을 두지 않는다.

추가 불변 조건은 다음과 같다.

- `team_works.issue_id`는 필수이며 상위 이슈 없는 팀 작업을 저장할 수 없다.
- 팀 작업의 프로젝트는 `issues.project_id`로만 결정하고 중복 저장하지 않는다.
- `(issue.project_id, project_team_id, team_id)`는 같은 워크스페이스의 `project_teams` 참여 관계와 일치해야 한다.
- 담당자와 워크플로 상태는 팀 작업의 워크스페이스·팀과 일치해야 한다.
- 같은 이슈·참여 팀에는 동시에 활성인 팀 작업을 여러 개 둘 수 있다. 재작업과 병렬 분할은 관계와 표시 ID로 구분하며 클라이언트가 별도 제목을 만들지는 않는다.
- 보관·삭제된 프로젝트, 보관된 팀과 삭제된 상태에는 새 팀 작업을 연결할 수 없다.

### 8.2.1 운영 배포 상태와 조건

운영 배포는 팀 워크플로 상태와 별도 축이다. `project_teams.deployment_tracking_enabled`는 새 TeamWork를 만들 때만 스냅샷 기준으로 사용한다. 값이 켜진 참여 팀의 새 작업은 `PENDING`, 꺼진 팀과 기존 작업은 `NOT_APPLICABLE`로 시작한다. 이후 프로젝트 설정을 바꿔도 이미 생성된 TeamWork의 배포 상태와 이력은 소급 변경하지 않는다.

별도 조건이 없으면 `각각 배포`다. 순서가 필요하면 `team_work_deployment_dependencies`에 `dependent_team_work_id`와 `predecessor_team_work_id`를 저장한다. 숫자 순번이나 전체 정렬을 저장하지 않으며 같은 이슈·워크스페이스, 자기 참조 금지, 관계 쌍 유일성과 전체 비순환을 강제한다.

같은 시점에 배포해야 하는 작업은 `deployment_groups` 한 건을 공유한다. 한 작업은 최대 한 그룹에만 속하고 유효 그룹은 TeamWork 두 건 이상이다. 함께 배포는 각 팀의 배포 실행을 한 요청으로 합치지 않는다. 그룹의 모든 팀 작업이 완료된 뒤 각 팀이 자기 배포 완료를 별도로 기록한다.

배포 완료는 TeamWork가 `COMPLETED`이고 모든 선행 작업이 `DEPLOYED`이며 함께 배포 그룹의 모든 작업도 `COMPLETED`일 때만 허용한다. 배포 완료된 TeamWork를 다시 미완료 상태로 바꾸면 `REDEPLOY_REQUIRED`가 된다. 취소되지 않은 모든 유효 팀 작업과 배포 대상 작업이 완료되면 이슈는 같은 트랜잭션에서 자동으로 `DONE`이 된다.

### 8.3 표시 ID 발급과 생성

이슈 생성은 워크스페이스의 `next_issue_number`, 팀 작업 생성은 팀의 `next_issue_number`를 원자적으로 증가시킨다. 카운터 증가와 대상 행 삽입은 같은 트랜잭션에서 실행한다.

이슈 생성 요청의 `initialTeams`가 생략되거나 비어 있으면 `UNSORTED` 이슈만 만든다. 참여 팀을 선택하면 활성 `project_teams`와 시작 상태를 잠그고 각 팀의 `team_works`를 함께 만든다. 팀 작업은 상위 이슈 콘텐츠를 복제하지 않으며 담당자가 있으면 가장 빠른 `UNSTARTED`, 없으면 기본 비종료 상태를 사용한다. 하나 이상 만들면 이슈 상태를 같은 트랜잭션에서 계산한다.

기존 이슈의 `작업 시작`은 선택 참여 팀별 기존 미완료 팀 작업을 재사용하고 없는 팀만 생성한다. 완료·취소된 작업만 있으면 새 팀 작업을 만들 수 있다. 복수 팀 중 하나라도 실패하면 카운터 증가와 모든 새 팀 작업을 롤백한다.

`teamAssignments`의 각 항목은 `projectTeamId`와 선택 `assigneeMembershipId`를 사용한다. 담당자가 있으면 해당 실제 팀의 활성 멤버인지 잠금 뒤 검증한다. `requireCurrentUserTeamMembership = true`는 서버가 참여 관계의 팀을 다시 확인하고 현재 멤버십을 검증한다.
...
참여 팀의 종류나 이름으로 준비 상태와 선후 순서를 계산하지 않는다. 모든 활성 참여 팀은 병렬로 TeamWork를 시작하고 자기 팀의 유효한 워크플로 상태로 즉시 전이할 수 있다. 명시적인 작업 전달만 원본·대상 연결을 만들며 범용 의존성 그래프는 만들지 않는다.
...
4. 유효 작업이 하나 이상이고 모두 `COMPLETED`지만 배포 대상 작업이 남으면 `REVIEW`다.
5. 유효 작업이 하나 이상이고 모두 `COMPLETED`이며 모든 배포 대상 작업이 `DEPLOYED`면 `DONE`이다. 배포 비대상 작업은 이 조건을 충족한 것으로 본다.
...
`PAUSED`, `CANCELED`에서는 자동 계산을 중지한다. `RESUME`은 `PAUSED`, `REOPEN`은 `CANCELED` 또는 `DONE`을 현재 팀 작업과 배포 상태 기준 계산값으로 되돌린다. `DONE`에서도 새 작업이나 재배포 필요 상태가 생기면 서버가 계산 상태로 되돌린다. 완료·취소된 역할 작업을 다시 시작하려면 이슈를 먼저 다시 열거나 재개해야 한다.
...
| `comment_id`, `team_work_id`, `api_handoff_id` | 모두 `NULL`이면 이슈 설명, 하나가 있으면 각각 댓글·작업 메모·작업 전달 멘션. 동시에 둘 이상을 가질 수 없음 |
| `mentioned_membership_id` | 저장 당시 활성 워크스페이스 멤버 |
| `created_at` | 관계 생성 시각 |

멘션 관계는 이슈 설명·댓글·작업 메모·작업 전달에 만든다. 설명, 댓글과 작업 메모를 저장할 때는 각 본문의 멘션 집합을 Markdown 파싱 결과와 한 트랜잭션에서 맞춘다. 추가 전용 작업 전달은 생성 시 멘션 관계를 함께 만든다. 같은 본문에서 같은 멤버를 여러 번 멘션해도 관계와 알림은 한 건이며 앵커별 멤버십 유일 제약으로 중복을 막는다.
...
| `source_team_work_id` | 전달을 작성한 원본 팀 작업 |
...
`api_handoff_targets`는 `(handoff_id, team_work_id)`를 복합 기본 키로 사용한다. 원본과 대상 팀 작업은 같은 이슈·프로젝트·워크스페이스에 속하고 서로 다른 활성 참여 팀을 참조해야 한다.

작업 전달은 수정·삭제하지 않는 추가 전용 데이터다. 원본 팀 작업별 `INITIAL`은 최대 한 건이고 `FOLLOW_UP`은 최초 전달 뒤에만 추가한다. API 명세 링크와 변경 요약 등 템플릿 제목만 있고 실질 내용이 없는 본문은 유효한 전달로 인정하지 않는다. 본문 멘션은 전달별 관계로 저장하고 활성 워크스페이스 멤버만 허용한다. 멘션 대상은 일반 전달 후보와 합친 뒤 수신자별 한 건으로 중복 제거하며 `MENTIONED` 유형을 우선한다.

모든 TeamWork는 `COMPLETE_ONLY`로 현재 작업만 완료하거나 `HANDOFF_AND_COMPLETE`로 다른 활성 참여 팀에 전달한 뒤 완료할 수 있다. 완료 요청의 `destinationProjectTeamIds`는 저장 컬럼이 아닌 트랜잭션 입력이며 자기 자신, 비활성 관계와 다른 프로젝트·워크스페이스 관계를 허용하지 않는다.

전달 트랜잭션은 원본 팀 작업과 이슈, 대상 참여 관계를 잠그고 팀별 삭제되지 않은 작업을 조회한다. 미완료 작업이 있으면 재사용하고 없으면 대상 팀의 기본 워크플로 상태로 미할당 작업을 생성한다. 기존 작업의 프로젝트·참여 관계·팀이 다르면 자동 수정하지 않는다. 전달 대상은 모두 `api_handoff_targets`에 명시한다.

전달, 대상 연결, 원본 상태 변경, 대상 작업 생성·재사용, 이슈 자동 상태, 활동과 Outbox 발행은 함께 커밋한다. 이미 처리된 동일 완료 요청은 새 전달·작업·이벤트를 만들지 않고 버전 충돌을 반환하며 웹은 최신 이슈를 조회해 성공 결과를 복구한다. `FOLLOW_UP`은 최초 전달의 대상 작업에 연결하고 새 팀 작업을 자동 생성하지 않는다.
...
관리자 CSV 가져오기의 검증·실행 결과만 저장한다. 원본 파일, 원본 행, 컬럼·값 매핑 JSON과 이슈 제목·설명 사본은 저장하지 않는다.

| 핵심 컬럼 | 규칙 |
| --- | --- |
| `id`, `workspace_id`, `requested_by_membership_id` | 실행 기록과 요청 관리자 |
| `execution_id` | 클라이언트가 한 흐름에서 유지하는 UUID. 워크스페이스 안에서 유일 |
| `source_fingerprint` | 파일 바이트의 SHA-256. 같은 워크스페이스의 완료 파일 중복 판정 |
| `status` | 미리보기, 검증 실패·성공, 처리 중, 전체 성공·실패 |
| `input_row_count`, `project_created_count`, `issue_created_count`, `connection_created_count`, `excluded_row_count`, `error_count` | 비음수 처리 수 |
| `last_error_code`, `error_details` | 정제된 오류 코드와 최대 200건의 행 번호·필드·심각도. 원본 값 금지 |
| `validated_target_fingerprint`, `validation_signature` | 검증한 활성 매핑 대상과 파일·매핑 조합의 SHA-256 |
| `started_at`, `completed_at`, `failed_at`, `created_at`, `updated_at` | 실행·결과 시각 |

`VALIDATED` 실행만 저장을 시작할 수 있다. 실행 전 워크스페이스를 잠그고 같은 실행 ID를 `PROCESSING`으로 한 번만 전이한다. 성공 상태는 늦게 도착한 중복 요청이 실패로 덮어쓸 수 없다.

### 10.6 `import_source_rows`

성공한 CSV 행과 생성 이슈의 중복 방지·추적 연결이다.

| 핵심 컬럼 | 규칙 |
| --- | --- |
| `id`, `workspace_id`, `import_run_id` | source 행과 성공 실행 |
| `source_key_hash` | NFC·trim·소문자 정규화 source key의 SHA-256 |
| `source_reference` | 관리자가 원본 시스템과 대조할 최대 255자 source key |
| `project_id`, `project_created`, `issue_id` | 연결 프로젝트, 이번 실행 생성 여부와 생성 이슈 |
| `created_at` | 저장 시각 |

`(workspace_id, source_key_hash)`를 유일하게 해 파일명이 달라도 같은 source key를 다시 만들지 않는다. `source_reference`는 제목·설명·댓글 같은 업무 본문이 아니며 가져오기 결과 대조에만 노출한다. 원본 CSV와 설명은 이 테이블에 저장하지 않는다.

### 10.7 `saved_views`

개인 저장된 보기는 소유 membership과 workspace에만 속하며 팀 공유나 구독 관계를 만들지 않는다. `resource_type`은 `ISSUES`, `MY_WORK`만 허용하고, `configuration`에는 서버 허용 목록의 검색·필터·정렬·표시 옵션만 정규화된 JSON으로 저장한다. `ISSUES` 다중 정렬은 `sorts` 배열의 `{ field, direction }` 객체 순서로 저장하고 `MY_WORK`는 단일 정렬 키를 유지한다. 임의 URL, SQL, 미지원 속성과 중첩 OR 구조는 저장하지 않는다.

| 핵심 컬럼 | 규칙 |
| --- | --- |
| `workspace_id`, `membership_id` | 현재 활성 workspace와 소유 membership. 모든 직접 조회·수정 조건에 함께 사용 |
| `resource_type`, `name`, `normalized_name` | 목록 종류와 NFC·trim·소문자 정규화 이름. membership·종류 안에서 유일 |
| `configuration` | 허용된 query configuration만 저장하는 JSONB |
| `is_default`, `version` | 종류별 기본 보기 하나와 낙관적 동시 수정 제어 |

부분 유일 인덱스 `saved_views_one_default_per_resource`가 `(membership_id, resource_type)`의 `is_default=true`를 하나로 제한한다. 기본 지정은 기존 기본 해제와 새 기본 지정을 한 PostgreSQL 트랜잭션으로 처리한다.

### 10.8 `issue_templates`와 `issue_template_labels`

이슈 템플릿은 워크스페이스 공용 생성 입력 스냅샷이다. `issues`에는 템플릿 외래 키나 version을 저장하지 않으며, 템플릿 수정·보관은 이미 생성된 이슈·라벨·팀 작업을 갱신하지 않는다. 템플릿 Markdown은 멘션을 문자열로 보존하지만 첨부 관계를 소유하지 않으므로 업로드 파일 참조는 저장하지 않는다.

| 핵심 컬럼 | 규칙 |
| --- | --- |
| `workspace_id`, `name`, `normalized_name` | 워크스페이스 소유와 NFC·trim·소문자 이름. 활성 템플릿 안에서 이름 유일 |
| `description_markdown`, `priority` | 필수 Markdown 본문과 기본 우선순위 |
| `project_id`, `initial_role` | 둘 다 선택이며 역할이 있으면 같은 워크스페이스 프로젝트가 반드시 존재 |
| `archived_at`, `version` | 신규 선택 제외 상태와 낙관적 동시 수정 제어 |

`issue_template_labels`는 템플릿과 라벨을 `(workspace_id, issue_template_id, label_id)`로 연결한다. 템플릿의 프로젝트·라벨은 복합 외래 키로 같은 워크스페이스만 허용한다. 역할 팀과 대상 보관 상태는 적용과 최종 이슈 생성 시 서버가 현재 값으로 재검증한다.

프로젝트가 휴지통에 있는 30일 동안에는 템플릿의 프로젝트·역할 값을 보존해 복구 시 원래 설정을 되살린다. 영구 삭제 트랜잭션은 프로젝트 행을 지우기 전에 이를 참조하는 템플릿의 `project_id`, `initial_role`을 함께 `NULL`로 바꾸고 `version`을 증가시킨다. 라벨 물리 삭제는 템플릿 연결에서 `RESTRICT`하며 현재 제품에서는 보관만 제공한다.

### 10.9 `product_feedback`

| 필드 | 계약 |
| --- | --- |
| `id`, `submission_id`, `workspace_id`, `submitted_by_membership_id` | 접수 UUID, 브라우저 재시도 UUID와 복합 workspace·제출자 소유권 |
| `category`, `body`, `current_path`, `release_id` | `BUG`, `USABILITY`, `IDEA`, `OTHER`, 정성 본문, query·fragment를 제외한 pathname과 서버 release ID |
| `status`, `status_changed_at`, `status_changed_by_membership_id`, `version` | `RECEIVED`, `IN_REVIEW`, `IMPLEMENTED`, `DEFERRED`와 관리자 변경·낙관적 잠금 |
| `retention_expires_at`, `created_at`, `updated_at` | 접수 후 1년 보존 만료와 감사 시각 |

`(workspace_id, submitted_by_membership_id, submission_id)`는 유일하다. 같은 식별자·같은 정규화 입력은 기존 행을 반환하고 다른 입력은 충돌한다. 제출자와 상태 변경자는 `(workspace_id, membership_id)` 복합 외래 키로 격리하며 membership 비활성화로 행을 삭제하지 않는다. maintenance는 `retention_expires_at < now()`인 행만 오래된 순서의 제한 batch로 삭제한다.

### 10.10 `product_event_states`

| 필드 | 계약 |
| --- | --- |
| `workspace_id`, `membership_id`, `semantic_key` | 현재 workspace의 활성 membership과 상태 종류를 식별하는 유일 키 |
| `state_value`, `version`, `updated_at` | 최신 상태, 실제 상태 전환 횟수와 서버 사건 시각 |

Alpha에서는 `push-permission` 상태만 저장한다. 원자적 upsert는 같은 결과 재제출을 무시하고 값이 바뀔 때만 version을 증가시킨다. 상태 전환 제품 이벤트 ID는 membership·semantic key·version·결과로 결정하며, workspace 또는 membership 물리 삭제는 상태 행을 먼저 정리하지 않으면 제한된다.
...
| 활성 팀·라벨·이슈 템플릿 이름 유일 | `archived_at IS NULL` 부분 유일 인덱스 |
...
| 프로젝트 참여 팀 중복 금지 | `project_teams(project_id, team_id)` 유일 제약 |
...
| 가져오기 실행 중복 금지 | `import_runs(workspace_id, execution_id)` 유일 제약 |
| 가져오기 source 중복 금지 | `import_source_rows(workspace_id, source_key_hash)`, `(workspace_id, issue_id)` 유일 제약 |
| 가져오기 처리 수 | 모든 실행 count 컬럼의 비음수 검사 제약 |
| 개인 보기 이름·기본 보기 | `(membership_id, resource_type, normalized_name)` 유일 제약과 종류별 `is_default` 부분 유일 인덱스 |
| 템플릿 기본 팀의 프로젝트 필수 | `initial_project_team_id`가 템플릿 프로젝트의 참여 관계를 참조하는 외래 키 |
| 제품 이벤트 상태 중복 금지 | `(workspace_id, membership_id, semantic_key)` 유일 제약과 상태 전환 upsert |

### 11.2 서버 트랜잭션에서 강제

- 이메일 인증, 초대와 비밀번호 재설정 토큰의 대상·목적·만료·일회 사용
- 초대 이메일과 로그인 이메일 일치, 기존 멤버십 부재
- TeamWork를 시작할 때 프로젝트에 활성 참여 팀이 하나 이상 존재
- 프로젝트 참여 관계와 팀 작업의 프로젝트·팀 일치
- 팀 작업이 상위 이슈와 같은 워크스페이스에 속하는지 확인
- 팀 작업 상태가 소속 팀의 상태인지 확인
- 활성 프로젝트 참여 팀이 기본 상태 하나와 담당자 배정에 필요한 `UNSTARTED` 상태를 유지하는지 확인
- 담당자가 활성 멤버십이면서 현재 팀 멤버인지 확인
- `initialTeams`와 `teamAssignments`의 참여 관계 중복 부재와 현재 프로젝트의 활성 관계 확인
- `requireCurrentUserTeamMembership` 요청은 참여 관계의 실제 팀에 현재 사용자가 속하는지 확인
- 내가 맡기의 후보 작업이 같은 이슈·참여 팀의 미할당 미완료 작업인지 확인하고 다른 담당자가 생긴 경우 변경하지 않음
- 일괄 배정의 모든 작업 ID·버전·이슈·참여 팀·현재 담당자와 후보 활성 팀 멤버를 변경 전 전부 확인
- 팀 작업 관계의 양쪽 이슈·워크스페이스 일치와 전체 순환 부재
- 전달 후 완료의 대상이 같은 프로젝트의 다른 활성 참여 팀인지 확인
- 전달의 후행 작업 다건 재사용, 대상 연결, 완료·취소 범위 충돌과 현재 프로젝트 참여 관계·팀 일치
- 업로드 파일의 실제 크기·형식, 업로더와 파일 범위 검증
- 프로필 사진 파일의 사용자 소유·`USER_PROFILE` 범위 검증
- 프로젝트 로고 파일의 업로더·`WORKSPACE` 범위·워크스페이스·이미지 형식·미연결 상태 검증
...
- 미완료 작업이 있는 프로젝트 참여 팀 제외와 사용 중 상태 변경 제한
...
- CSV 파일 해시·검증 서명·활성 매핑 대상 fingerprint 일치, 관리자·워크스페이스·보관 상태 재검증
- 한 파일 안 source key 중복, 완료 파일 중복과 기존 `import_source_rows` 중복 판정
- 개인 보기의 허용 configuration, 소유 membership·workspace, version과 기본 보기 교체를 같은 요청에서 검증
- 이슈 템플릿 관리의 관리자 권한, version과 활성 이름 유일성, 현재 활성 라벨·프로젝트·참여 팀을 검증
- 템플릿 적용과 템플릿 기반 이슈 생성은 같은 ID·version·보관 상태와 현재 활성 대상을 다시 검증하되 템플릿 본문을 활동·Outbox·분석 payload에 복제하지 않음
...
| 기본 이슈 생성 | 프로젝트·참여 관계·시작 상태 잠금, 이슈와 선택한 `initialTeams`의 팀별 카운터 증가·TeamWork, 이슈 자동 상태, 라벨, 구독, 활동, Outbox. 빈 선택은 팀 작업 없는 `UNSORTED` |
| 이슈 작업 시작 | 이슈·참여 관계·시작 상태 잠금, `teamAssignments` 검증, 선택 팀의 기존 미완료 작업 재사용, 없는 팀의 카운터 증가·TeamWork, 이슈 자동 상태, 담당자·구독, 활동, 알림 후보·Outbox. `requireCurrentUserTeamMembership`이면 현재 팀 멤버십도 잠금·재검증 |
| 내가 맡기 | 이슈·참여 팀·현재 멤버십과 후보 작업 잠금, 후보 0건이면 새 작업 생성·1건이면 재사용·여러 건이면 선택 요구, 미할당 재검증, 자신에게 할당, 구독, 활동, 자기 알림을 제외한 Outbox |
| 팀 작업 일괄 배정 | 이슈와 모든 대상 팀 작업·버전·참여 팀·후보 멤버 잠금, 전체 검증 뒤 담당자·구독·활동·알림 후보·Outbox 일괄 반영. 한 건 충돌 시 전체 롤백 |
...
| 프로젝트 참여 팀 변경 | 권한·현재 version·미완료 작업 확인, 비활성화 또는 기존 관계 재활성화, 프로젝트 활동·버전, Outbox |
| 댓글 저장 | 이슈 댓글과 선택 팀 작업 문맥, 멘션 집합, 댓글 이미지 연결과 `unlinked_at`, 자동 구독, 활동, Outbox |
| 팀 작업 전달·완료 | 이슈·원본 작업·대상 참여 관계·시작 상태 잠금, 전달과 대상 연결, 전달 이미지 연결과 `unlinked_at`, 상태 변경, 대상 팀 작업 생성 또는 재사용, 이슈 진행률·자동 상태, 활동, 알림 후보 스냅샷과 Outbox |
...
| CSV 가져오기 | 워크스페이스 잠금, 검증 실행 claim, 새 라벨·프로젝트·프로젝트 참여 팀, 이슈·팀 작업·라벨 연결, source row, 가져오기 활동, 이슈·팀 표시 번호 카운터와 실행 성공 결과. 한 단계라도 실패하면 전체 롤백 |
| 이슈 템플릿 관리 | 현재 version 조건부 수정·보관, 라벨 연결 교체와 활성 이름 유일성 확인. 영향 행이 없으면 최신 version을 반환하고 덮어쓰지 않음 |
| 템플릿 기반 이슈 생성 | 템플릿 ID·version·보관 상태와 현재 라벨·프로젝트·기본 참여 팀을 다시 확인한 뒤 일반 이슈 생성 스냅샷만 저장. 무효 기본 팀은 입력을 보존하고 재선택을 요구함 |
| 템플릿이 참조한 프로젝트 영구 삭제 | 복구 가능 이슈 부재 확인 뒤 참조 템플릿의 프로젝트·최초 역할 해제와 version 증가, 프로젝트 역할·활동·프로젝트 삭제. 복구 기간에는 템플릿 참조 유지 |
...
| 팀 | 삭제 없음. 조건을 만족하면 보관 |
| 이슈 템플릿 | 삭제 없음. 보관하면 신규 선택에서만 제외하고 기존 이슈에는 영향 없음 |
| 라벨 | 보관, 기존 이슈 연결 유지 |
...
| 내보내기 실행 기록 | 30일 |
| 제품 피드백 | 접수 후 1년, 만료 뒤 다음 일일 정리까지 |
...
| 이슈 템플릿 목록 | `(workspace_id, archived_at, updated_at, id)`와 활성 이름 부분 유일 인덱스 |
...
- B3는 실데이터와 구버전 프로세스가 존재할 수 있으므로 expand/backfill/contract로 배포한다. expand 전에 같은 프로젝트에서 한 팀이 여러 구 역할을 맡은 데이터를 preflight로 차단하고 역할별 팀을 분리하거나 중복 역할 배정을 제거한다. expand 단계에서는 `project_teams`, `team_works.project_team_id`, `issue_templates.initial_project_team_id`를 추가하고 기존 프로젝트·팀 조합과 TeamWork·템플릿을 검증하며 backfill한다.
- expand 기간에는 구버전 프로세스의 `ProjectRoleTeam`, `TeamWork.projectRole`, `IssueTemplate.initialRole` 쓰기를 호환 트리거로 새 관계에 반영한다. 새 API·Worker·Web은 역할 기반 쓰기를 제공하지 않으며 `projectRole`은 서버 조회에도 노출하지 않는다.
- 새 버전 프로세스 전환과 누락·교차 workspace·담당자·워크플로·전달 대상 검증이 끝난 뒤 별도 contract migration에서 구 enum, 표·컬럼과 호환 트리거를 제거한다. 이 제거 조건이 충족되기 전 destructive migration을 같은 배포에 포함하지 않는다.
- 기존 마이그레이션 파일은 수정하지 않고 새 마이그레이션을 추가한다. 로컬 `rivet_test` 테스트 데이터베이스는 필요하면 초기화한 뒤 처음부터 마이그레이션을 재적용한다.
...
| 최초 작업 생성 | 생략·빈 선택은 이슈만, 단일·복수 `initialTeams`는 선택 참여 팀 작업만 이슈와 함께 생성되고 일부 실패 시 모두 롤백됨 |
| 작업 시작 | 팀 작업 없는 이슈에서 단일·복수 참여 팀을 시작하고 기존 미완료 팀 작업을 중복 생성하지 않으며 일부 실패 시 모두 롤백됨 |
| 이슈 자동 상태 | 작업 없음·시작 전만·일부 시작 또는 완료·전체 작업 완료와 배포 잔여·전체 작업과 배포 완료·전체 취소 조합이 각각 `UNSORTED`, `TODO`, `IN_PROGRESS`, `REVIEW`, `DONE`, `UNSORTED`로 저장되고 팀 작업·배포 변경과 이슈 상태가 함께 커밋됨 |
| 이슈 상태 행동 | `PAUSED`, `CANCELED`에서 자동 전이가 멈추고 재개·다시 열기는 현재 팀 작업과 배포 상태로 재계산되며 닫힌 이슈의 작업 시작이 거부됨 |
| 참여 팀 담당자 시작 | `teamAssignments`의 참여 관계와 실제 팀 활성 멤버 검증, 복수 팀 전체 커밋·롤백 |
...
| 프로젝트 참여 팀 | 동일 프로젝트·팀 중복 관계, 다른 프로젝트·워크스페이스 관계와 팀이 다른 TeamWork를 연결할 수 없음 |
| 상위 이슈 | 이슈 없는 팀 작업 생성과 다른 워크스페이스 이슈 연결을 거부함 |
| 작업 전달 | 자기 자신·비활성·다른 프로젝트·워크스페이스 대상을 거부하고 대상 작업 생성·재사용·연결·이슈 진행률이 원자적으로 반영됨 |
| 전달 조회 | 이슈와 전달받은 팀 작업 문맥이 같은 `ApiHandoff` 원본과 대상 관계를 조회함 |
| 전달 재시도 | 같은 완료 요청 재시도에 전달·대상 팀 작업·Outbox가 중복되지 않고 추가 전달은 팀 작업을 만들지 않음 |
...
| 개인 저장된 보기 | 이름 충돌·오래된 version·동시 기본 지정이 최신 보기를 덮지 않으며 다른 membership·workspace ID는 404 |
