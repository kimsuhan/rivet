...
`API_HANDOFF_CREATED`는 기존 `schemaVersion: 1`과 위 필드를 그대로 사용한다. 수신자별 알림 이동 대상은 워커가 `downstreamIssueIds`와 현재 관계를 이용해 계산하므로 별도 대상 ID나 전달 본문을 payload에 추가하지 않는다. 같은 이벤트를 재처리할 때도 payload 버전과 멱등 키를 바꾸지 않는다.
...
역할별 담당자를 포함한 작업 시작, 내가 맡기와 일괄 배정도 새 이벤트나 알림 유형을 추가하지 않는다. 새 팀 작업은 `ISSUE_CREATED`, 기존 작업 담당자 변경은 `ISSUE_CHANGED`를 사용하고 같은 팀 작업의 한 배정 사건에서 같은 aggregate와 의미의 Outbox를 여러 건 만들지 않는다. 일괄 배정은 변경된 팀 작업별 정본 이벤트를 만들 수 있지만 같은 작업·수신자 조합을 중복 생성하지 않는다.
...
작업 전달 이벤트에는 멘션 ID를 넣지 않는다. 전달 본문의 멘션 참조는 API에서 거부하므로 작업 전달 멘션 관계와 `MENTIONED` 알림 수신자를 만들지 않는다. 최초 전달에서 재사용한 실제 미완료 후행 작업은 담당자와 자동 구독자, 새로 생성한 미할당 후행 작업은 대상 팀의 사건 당시 활성 멤버를 후보로 사용한다. 추가 전달은 기존 후행 작업의 담당자와 자동 구독자만 사용한다.
...
| 최초 전달 | 재사용한 미완료 후행 작업의 담당자·자동 구독자 + 새 미할당 후행 작업의 대상 팀 활성 멤버 스냅샷 | `API_HANDOFF_CREATED` |
| 추가 전달 | 기존 미완료 후행 작업의 담당자·자동 구독자 스냅샷 | `API_HANDOFF_FOLLOW_UP_CREATED` |

한 `COMMENT_CREATED` 이벤트에서 같은 사용자가 멘션과 구독에 모두 해당하면 `MENTIONED` 하나만 만든다. 최초 전달은 트랜잭션에서 실제 생성 또는 재사용한 후행 웹·앱 팀 작업만 대상으로 한다. 최초 전달에서 새 미할당 작업을 만들면 담당자·구독자가 없어도 팀 활성 멤버가 후보가 된다. 추가 전달은 같은 부모와 프로젝트에 이미 존재하는 미완료 웹·앱 작업만 대상으로 하며 프로젝트 역할의 팀 전체로 넓히지 않는다.

작업 시작·내가 맡기·일괄 배정의 새 담당자는 기존 `ISSUE_ASSIGNED` 후보가 된다. `actor_membership_id`와 새 담당자가 같으면 공통 수신 제외 규칙으로 알림을 만들지 않으며, 담당자 없는 우리 팀 시작은 할당 알림을 만들지 않는다.
...
2. 최초 전달이면 `destinationRoles`별 기존 미완료 작업을 모두 재사용하고, 기존 작업이 없으면 미할당 작업을 생성한다. `BLOCKS` 관계는 새로 만든 작업에만 자동 생성하고 기존 작업은 병렬 상태와 사용자가 만든 관계를 유지한다. 완료·취소 작업만 있거나 범위가 불일치하면 Outbox를 포함한 전체 트랜잭션을 롤백한다.
3. 재사용 작업은 현재 담당자와 `issue_subscriptions` 멤버십을 합친다.
4. 새 미할당 작업은 해당 역할 팀의 사건 당시 활성 `team_members` 멤버십을 합친다.
5. 추가 전달이면 새 작업과 관계를 만들지 않고 같은 부모와 프로젝트의 기존 미완료 웹·앱 작업에서 담당자와 자동 구독자만 합친다.
6. 작성자를 제외하고 후행 이슈 ID와 후보 멤버십 ID를 각각 중복 제거해 payload에 저장한다.

작업 전달, 백엔드 완료, 후행 작업·관계, 활동, 후보 스냅샷과 Outbox는 같은 트랜잭션에서 커밋한다. 커밋 뒤 후행 담당자나 팀 멤버 구성이 바뀌어도 사건 당시 후보를 바꾸지 않는다. 다만 워커 처리 전에 멤버가 비활성화되면 알림을 생성하지 않는다.
...
| 최초·추가 작업 전달 | 수신자와 연결된 후행 작업, 없으면 상위 이슈, 없으면 원본 백엔드 작업 | `NULL` | 해당 전달 |

작업 전달 알림의 `issue_id`는 `ApiHandoff` 소유 이슈가 아니라 수신자별 이동 대상이다. 워커는 삭제되지 않은 `downstreamIssueIds` 중 다음 우선순위로 한 건을 선택한다.

1. 수신자가 현재 담당자인 후행 작업
2. 수신자가 자동 구독자인 후행 작업
3. 수신자가 대상 팀의 활성 멤버인 후행 작업

같은 우선순위 후보가 여러 건이면 프로젝트 역할, 표시 식별자와 ID 순으로 안정 정렬한다. 일치하는 후행 작업이 없으면 원본 백엔드 작업의 `parent_issue_id`, 상위 이슈도 없으면 payload의 원본 백엔드 `issueId`를 사용한다. 어떤 `issue_id`를 선택해도 `handoff_id`는 같은 원본 전달을 유지한다.

웹은 후행 작업이면 `전달받은 내용`, 상위 이슈면 `작업 흐름`, 원본 백엔드 작업이면 전달 영역의 동일 `handoff_id` 앵커를 연다. 이 세 화면은 `api_handoffs` 원본과 기존 관계를 함께 조회하며 알림, 활동 또는 화면별 전달 사본을 만들지 않는다. 최초 전달 뒤 추가 전달이 생기면 같은 원본 흐름을 다시 조회해 상위 이슈와 관련 프론트 작업 양쪽에 함께 반영한다.
...
- 전달 알림의 수신자별 `issue_id` 선택도 이 트랜잭션 안에서 수행한다. 같은 이벤트를 재처리하면 `(event_id, recipient_membership_id)` 유일 제약으로 기존 알림과 이동 대상을 유지하고 새 알림을 만들지 않는다.
...
- 알림 이동 대상 계산을 바꿔도 기존 `API_HANDOFF_CREATED` payload v1과 `(event_id, recipient_membership_id)` 멱등 키가 유지된다.
...
- 전달 알림은 수신자별 담당 후행 작업, 구독 후행 작업, 활성 팀 멤버인 후행 작업 순으로 이동하며 후보가 없으면 상위 이슈와 원본 백엔드 작업 순으로 대체된다.
- 모든 이동 대상은 같은 `handoff_id`를 사용하고 상위 이슈와 프론트 작업은 원본 `ApiHandoff`의 최초·추가 전달을 함께 조회한다.
- 같은 Outbox 이벤트 재처리에도 `(event_id, recipient_membership_id)`가 중복되지 않는다.
- 읽음·읽지 않음과 모두 읽음 뒤 목록과 전역 배지 수가 일치한다.
- 역할별 작업 시작과 일괄 배정의 새 담당자에게 기존 `ISSUE_ASSIGNED`가 팀 작업별 한 건만 생성되고, 내가 맡기처럼 행위자와 담당자가 같은 경우에는 생성되지 않는다.
