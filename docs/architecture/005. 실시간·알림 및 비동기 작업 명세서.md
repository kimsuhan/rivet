...
| 항목 | 내용 |
| --- | --- |
| 문서 상태 | 확정 — MVP 실시간·알림·비동기 작업과 Closed Alpha A3~A5 구현 기준 |
| 문서 버전 | v0.20 |
| 작성일 | 2026-07-18 |
| 대상 릴리스 | Rivet MVP, Closed Alpha A3~A5 |
...
참여 팀별 담당자를 포함한 작업 시작, 내가 맡기와 일괄 배정은 `TEAM_WORK_CREATED`, `TEAM_WORK_CHANGED`와 `TEAM_WORK_ASSIGNED`를 사용한다. 같은 팀 작업의 한 배정 사건에서 같은 aggregate와 의미의 Outbox를 여러 건 만들지 않는다. 일괄 배정은 변경된 팀 작업별 정본 이벤트를 만들 수 있지만 같은 작업·수신자 조합을 중복 생성하지 않는다.
...
작업 전달 이벤트는 본문의 `mentionedMembershipIds`를 포함한다. 최초 전달의 대상 참여 팀 작업은 담당자와 자동 구독자, 새 미할당 작업은 대상 팀의 사건 당시 활성 멤버를 일반 후보로 사용하고, 추가 전달은 최초 전달 대상 작업의 담당자와 자동 구독자만 사용한다. 워커는 일반 후보와 멘션 대상을 합치고 작성자·비활성 멤버를 제외한 뒤 멘션 대상에는 `MENTIONED` 유형을 우선한다.
...
| `PROJECT_CREATED` | `PROJECT` | 참여 팀 수, 목표일 설정 여부 | `project_created` 분석 |
...
| 최초 전달 | 본문 멘션 + 대상 참여 팀 작업의 담당자·자동 구독자 + 새 미할당 작업의 대상 팀 활성 멤버 스냅샷 | 멘션 대상은 `MENTIONED`, 나머지는 `API_HANDOFF_CREATED` |
| 추가 전달 | 본문 멘션 + 최초 전달 대상 팀 작업의 담당자·자동 구독자 스냅샷 | 멘션 대상은 `MENTIONED`, 나머지는 `API_HANDOFF_FOLLOW_UP_CREATED` |

같은 사용자가 한 사건의 멘션과 구독·할당·전달 후보에 함께 해당하면 `MENTIONED` 하나만 만든다. 최초 전달은 트랜잭션에서 실제 생성 또는 재사용한 후행 참여 팀 작업만 일반 전달 대상으로 한다. 새 미할당 작업을 만들면 담당자·구독자가 없어도 대상 팀 활성 멤버가 후보가 된다. 추가 전달은 최초 전달에 연결된 대상 작업의 담당자·구독자만 사용하고 프로젝트의 다른 참여 팀 전체로 넓히지 않는다. 명시적인 본문 멘션 대상은 이 일반 후보 제한과 별개로 활성 워크스페이스 멤버면 수신자가 된다.
...
1. 원본 TeamWork가 현재 프로젝트 참여 관계와 실제 팀을 일치하게 참조하는지 확인한다.
2. 최초 전달이면 `destinationProjectTeamIds`가 같은 프로젝트의 다른 활성 참여 팀인지 확인하고 기존 미완료 작업을 재사용하며, 없으면 기본 비종료 상태의 미할당 작업을 생성한다. 모든 대상은 `api_handoff_targets`에 저장한다.
3. 재사용 작업은 현재 담당자와 `issue_subscriptions` 멤버십을 합친다.
4. 새 미할당 작업은 해당 실제 팀의 사건 당시 활성 `team_members` 멤버십을 합친다.
...
작업 전달, 원본 완료, 대상 참여 팀 작업 생성·재사용, 활동, 후보 스냅샷과 Outbox는 같은 트랜잭션에서 커밋한다. 커밋 뒤 대상 작업 담당자나 팀 멤버 구성이 바뀌어도 사건 당시 후보를 바꾸지 않는다. 다만 워커 처리 전에 멤버가 비활성화되면 알림을 생성하지 않는다.
...
4. 이슈 템플릿 기본 팀 참조를 해제하고 참여 팀 관계, 구버전 역할 관계, 활동과 프로젝트 행을 하나의 트랜잭션에서 제거한다.
...
| 제품 피드백 | `retention_expires_at < now()`, 접수 후 1년 |
...
- 참여 팀 작업 시작과 일괄 배정의 새 담당자에게 `TEAM_WORK_ASSIGNED`가 팀 작업별 한 건만 생성되고, 내가 맡기처럼 행위자와 담당자가 같은 경우에는 생성되지 않는다.
...
## 18. Closed Alpha 이슈 템플릿 이벤트 경계

템플릿 적용은 생성 입력을 채우는 읽기·검증 동작이며 별도 알림이나 실행 Outbox를 만들지 않는다. 템플릿 기반 이슈 생성은 기존 `ISSUE_CREATED` 트랜잭션과 Outbox를 사용하고, 이슈 활동의 생성 메타데이터에는 적용한 템플릿 ID와 version만 선택적으로 포함한다.

템플릿 설명 Markdown, 복사된 이슈 설명·제목, 라벨 이름, 프로젝트 이름과 팀 이름은 활동·SSE·Outbox·PostHog payload에 넣지 않는다. 기존 `ISSUE_CREATED` Outbox 허용 목록도 `issueId`, `mentionedMembershipIds`, `schemaVersion`으로 유지한다. A5에서 템플릿 제품 이벤트를 추가하더라도 별도 versioned 허용 목록에 ID·version·결과처럼 정제된 값만 정의한다.

템플릿 수정·보관은 이미 생성된 이슈의 활동·알림·Outbox를 만들거나 다시 보내지 않는다. 반복 일정 자동 생성, 자동화 실행, 외부 공유와 템플릿 변경 전파도 worker 책임에 포함하지 않는다.

## 19. Closed Alpha 제품 이벤트 계약

- 모든 이벤트는 불변 UUID `eventId`, `payloadVersion = 1`, ISO `occurredAt`, workspace·membership와 사건별 정확한 속성만 갖는다. PostHog 최상위 `uuid`에 `eventId`를 사용한다. 브라우저 요청은 이름과 속성만 보내며 API가 나머지 envelope을 만든다.
- Outbox 기반 사건은 원본 Outbox ID와 사건 이름으로 결정한 UUID를 사용한다. 한 Outbox가 여러 분석 사건을 만들 수 있으므로 이름별 UUID를 분리한다. 피드백·알림처럼 정본 ID가 사건을 유일하게 나타내면 해당 ID를 사용한다.
- 브라우저 사건은 성공 동작 뒤 최선 노력으로 `/product-events`에 전송한다. API는 saved view 소유 membership, 활성 템플릿, 알림 수신 membership과 검색 리소스의 workspace를 확인하고 membership당 분당 60건으로 제한한다. 분석 전용 Outbox는 만들지 않으며 네트워크·PostHog 실패는 제품 트랜잭션, Outbox 완료와 HTTP 성공 응답을 바꾸지 않는다.
- 허용 목록은 초대·직접 가입·로그인, CSV 단계, 이슈·작업 시작·완료, 저장 보기, 템플릿 생성·적용·기반 이슈, Push 권한·테스트·전달 결과·클릭, 검색, 알림과 피드백 사건을 포함한다. 각 속성은 안전한 UUID, enum, boolean 또는 비음수 count로 제한한다.
- 알림 생성은 워커가 원본 Outbox 처리 완료 뒤 알림 정본 ID로 전송하고, 읽음은 같은 알림 ID를 사용해 지연을 계산한다. 재조회·재처리의 같은 `eventId`는 한 사건으로 계산한다.
- 실제 Push 전달은 `WebPushDelivery`가 `SENT` 또는 `FAILED`로 저장된 뒤 delivery ID 기반 사건을 비차단 전송한다. 재시도 중 `PENDING` 상태는 실패 사건으로 세지 않고 실제 notification ID만 payload에 포함한다. 전달 건수는 delivery 단위로 보되 클릭 비율 분모는 한 개 이상 성공한 고유 notification, 분자는 그중 클릭한 고유 notification으로 계산한다.
- 이슈 생성·작업 시작·완료 사건은 같은 `issueId`를 가져 actor가 달라도 한 흐름으로 상관한다. CSV 검증은 요청마다 서버 `attemptId`와 정제 오류 코드 배열을 가져 실제 재검증과 매핑 오류를 계산하고, Push 권한 결과에는 `UNSUPPORTED`를 포함한다.
- 피드백 본문과 업무 콘텐츠, 이메일, 토큰, 파일명, 첨부 정보, Push endpoint는 payload에 존재할 수 없다. 계약 거부와 공급자 실패 로그도 해당 값을 복제하지 않는다.
