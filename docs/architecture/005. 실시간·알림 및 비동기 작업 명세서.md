...
| `TEAM_WORK_CHANGED` | `TEAM_WORK` | `issueId`, `teamWorkId`, `changedFields`, `assigneeMembershipId`, `stateCategory`, 분석용 제한 속성 | 할당 알림, 상태·담당 분석 |
...
| `API_HANDOFF_CREATED` | `API_HANDOFF` | `issueId`, `sourceTeamWorkId`, `handoffId`, `kind`, `downstreamTeamWorkIds`, `candidateRecipientMembershipIds`, 분석용 제한 속성 | 최초·추가 전달 알림과 분석 |

`API_HANDOFF_CREATED`는 위 필드를 갖는 새 schema version으로 전환한다. 수신자별 알림의 팀 작업 문맥은 워커가 `downstreamTeamWorkIds`와 현재 대상 관계를 이용해 계산하며 전달 본문을 payload에 넣지 않는다. 같은 이벤트를 재처리할 때는 payload 버전과 멱등 키를 바꾸지 않는다.

`changedFields`는 리소스별 허용 필드 enum의 중복 없는 배열이다. `assigneeMembershipId`는 팀 작업 담당자가 바뀐 경우에만 현재 값 또는 `null`, `stateCategory`는 팀 작업 상태가 바뀐 경우 현재 범주다. 설명·댓글 수정은 기존 멘션 전체가 아니라 저장 전후 차집합인 새 멘션 ID만 이벤트에 넣는다.

이슈 생성은 설명 멘션을 `ISSUE_CREATED`에, 함께 생성된 각 팀 작업은 `TEAM_WORK_CREATED`에 기록한다. 이슈 수정과 팀 작업 수정도 각 리소스의 변경 이벤트를 분리한다. 같은 수신자가 한 이벤트의 여러 규칙에 걸리면 `MENTIONED → TEAM_WORK_ASSIGNED → 완료·취소` 순서로 한 유형만 선택한다. 담당자가 `null`이면 할당 알림을 만들지 않는다.
...
작업 전달 이벤트에는 멘션 ID를 넣지 않는다. 전달 본문의 멘션 참조는 API에서 거부하므로 작업 전달 멘션 관계와 `MENTIONED` 알림 수신자를 만들지 않는다. 최초 전달 대상 프론트 작업은 담당자와 자동 구독자, 새 미할당 작업은 대상 팀의 사건 당시 활성 멤버를 후보로 사용한다. 추가 전달은 최초 전달 대상 작업의 담당자와 자동 구독자만 사용한다.
...
| 최초 전달 | 대상 프론트 작업의 담당자·자동 구독자 + 새 미할당 작업의 대상 팀 활성 멤버 스냅샷 | `API_HANDOFF_CREATED` |
| 추가 전달 | 최초 전달 대상 프론트 작업의 담당자·자동 구독자 스냅샷 | `API_HANDOFF_FOLLOW_UP_CREATED` |

한 `COMMENT_CREATED` 이벤트에서 같은 사용자가 멘션과 구독에 모두 해당하면 `MENTIONED` 하나만 만든다. 최초 전달은 트랜잭션에서 실제 생성 또는 재사용한 후행 웹·앱 팀 작업만 대상으로 한다. 최초 전달에서 새 미할당 작업을 만들면 담당자·구독자가 없어도 팀 활성 멤버가 후보가 된다. 추가 전달은 같은 이슈에 이미 존재하는 미완료 웹·앱 작업만 대상으로 하며 프로젝트 역할의 팀 전체로 넓히지 않는다.
...
6. 작성자를 제외하고 후행 팀 작업 ID와 후보 멤버십 ID를 각각 중복 제거해 payload에 저장한다.
...
Closed Alpha의 Web Push는 인앱 알림 레코드를 정본으로 재사용하는 별도 전달 채널로 설계한다. 수신자 계산을 복제하지 않고 브라우저 구독 수명주기, 권한 거절, 만료 구독 정리와 `(notification_id, subscription_id)` 멱등성을 추가한다. Slack은 반복 요구가 확인될 때 팀·프로젝트 공용 알림부터 별도 명세로 확정한다.
