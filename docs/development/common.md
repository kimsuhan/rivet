...
> 정본 책임: 코드 작성, 검증, 의존성과 공통화 기준
> 갱신 조건: 저장소 공통 구조, 품질 게이트 또는 개발 정책 변경
...
- `pnpm test:coverage`로 패키지별 line·branch·function 커버리지 리포트를 생성하되 저장소 전체
  퍼센트 threshold는 실제 기준선과 위험 근거 없이 출시 게이트로 강제하지 않는다.
