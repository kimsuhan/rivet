...
> Linear 마케팅 화면에서 추출한 시각적 특성과 원시 값을 보존한 참고 자료다. Rivet의 현재 디자인 방향은 저장소 루트의 [`DESIGN.md`](../../DESIGN.md), 정확한 제품 UI 구현 값은 [`005. 디자인 시스템 명세서`](../planning/005.%20디자인%20시스템%20명세서.md)를 따른다.
...
4. 원본 분석을 수정했다면 `npx @google/design.md lint "docs/reference/001. Linear 시각 디자인 분석.md"`로 검사한다.
