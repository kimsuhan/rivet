...
test('가입부터 이메일 인증과 기본 팀 생성까지 완료한다', async ({ page }, testInfo) => {
  ...
  try {
    ...
    await expect(page.getByRole('navigation', { name: '초기 설정 단계' })).toBeVisible();
    await checkLayout(page, testInfo.project.name, 'workspace-choice');
    await page.getByRole('button', { name: '초대받은 워크스페이스에 참여하기' }).click();
    await expect(page.getByRole('heading', { name: '초대를 기다리고 있어요' })).toBeVisible();
    await expect(page.getByText(email, { exact: true })).toBeVisible();
    await expect(page.getByText('초대 메일이 오지 않았나요?')).toBeVisible();
    await expect(page.getByRole('navigation', { name: '초기 설정 단계' })).toBeVisible();
    await checkLayout(page, testInfo.project.name, 'workspace-waiting');
    await page.getByRole('button', { name: '새 워크스페이스 만들기' }).click();
    await expect(page.getByRole('heading', { name: '워크스페이스 만들기' })).toBeVisible();
    await checkLayout(page, testInfo.project.name, 'workspace-create');
    await page.getByLabel('워크스페이스 이름').fill('Browser Workspace');
    await expect(page.getByLabel('슬러그')).toHaveValue('browser-workspace');
    ...
    await page.getByLabel('팀 이름').fill('web');
    await expect(page.getByLabel('팀 키')).toHaveValue('WEB');
