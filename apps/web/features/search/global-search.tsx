...
export function GlobalSearch({
  ...
  function openResult(result: (typeof results)[number]) {
    captureProductEvent('search_result_selected', {
      resourceId: result.resourceType === 'TEAM_WORK' ? result.teamWork!.id : result.issue.id,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent closeLabel={labels.close} className="gap-0 overflow-hidden p-0 sm:max-w-2xl">
        ...
        <div role="listbox" aria-label={labels.results} className="max-h-[60dvh] overflow-y-auto">
          {query.trim().length < 2 ? (
            <Empty className="min-h-64">
              <EmptyHeader>
                ...
                <EmptyTitle>{labels.minimumTitle}</EmptyTitle>
                <EmptyDescription>{labels.minimumDescription}</EmptyDescription>
              </EmptyHeader>
            </Empty>
          ) : null}
          {search.isPending && debounced.length >= 2 ? (
            <div aria-label={labels.loading} className="divide-y">
          ...
          {results.map((result) => {
            const work = result.teamWork;
            return (
              <button
                key={`${result.resourceType}-${work?.id ?? result.issue.id}`}
                type="button"
                role="option"
                aria-selected="false"
                className="hover:bg-muted/40 flex min-h-18 w-full items-center gap-3 border-b px-4 py-3 text-left"
                onClick={() => openResult(result)}
              >
                <Badge variant="outline">
                  {result.resourceType === 'TEAM_WORK' ? labels.teamWork : labels.issue}
                </Badge>
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-medium">
                    <span className="text-muted-foreground mr-2 font-mono text-xs">
                      {work?.identifier ?? result.issue.identifier}
                    </span>
                    {result.issue.title}
                  </span>
                ...
                {result.matchType === 'IDENTIFIER_EXACT' ? (
                  <Badge variant="secondary">{labels.exactMatch}</Badge>
                ) : null}
              </button>
            );
          })}
