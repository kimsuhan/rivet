...
export function ProjectDetailScreen({ projectId }: { projectId: string }) {
  ...
  return (
    <section className="mx-auto max-w-6xl space-y-6" aria-labelledby="project-title">
      <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-muted-foreground text-sm">프로젝트</p><h1 id="project-title" className="text-2xl font-semibold">{item.name}</h1><p className="text-muted-foreground mt-2 max-w-2xl text-sm">{item.description ?? '설명이 없습니다.'}</p></div><div className="flex gap-2"><Link href={`/projects/${item.id}/edit`} className={cn(buttonVariants({ variant: 'outline' }), 'gap-2')}><Pencil className="size-4" />편집</Link><Link href={`/projects/${item.id}?create=1&projectId=${encodeURIComponent(item.id)}`} className={cn(buttonVariants(), 'gap-2')}><Plus className="size-4" />이슈 만들기</Link></div></header>
      ...
      <div className="flex flex-wrap gap-2">{item.projectTeams.filter(({ active }) => active).map(({ id, team }) => <Badge key={id} variant="secondary"><span className="font-mono">{team.key}</span> · {team.name}</Badge>)}</div>
      ...
      {issues.data && issues.data.items.length === 0 ? <ContentEmpty icon={FolderKanban} title="프로젝트 이슈가 없습니다" description="이 프로젝트에서 첫 이슈를 만들어 보세요."><Link href={`/projects/${item.id}?create=1&projectId=${encodeURIComponent(item.id)}`}><Button><Plus className="size-4" />이슈 만들기</Button></Link></ContentEmpty> : null}
