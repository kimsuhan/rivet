...
export function IssueListRow({ issue, queryKey }: { issue: IssueSummaryResponseDto; queryKey: QueryKey }) {
  ...
  const mutation = useMutation({
    ...
    onMutate: async (priority) => {
      ...
      queryClient.setQueryData<IssueListResponseDto>(queryKey, (current) =>
        current ? { ...current, items: current.items.map((item) => item.id === issue.id ? { ...item, priority } : item) } : current,
      );
      return { previous };
    },
    onError: (_error, _priority, context) => queryClient.setQueryData(queryKey, context?.previous),
    onSuccess: (updated) => queryClient.setQueryData<IssueListResponseDto>(queryKey, (current) =>
      current ? { ...current, items: current.items.map((item) => item.id === issue.id ? updated : item) } : current,
    ),
    onSettled: () => void queryClient.invalidateQueries({ queryKey }),
  });
  const nextAction = issue.status === 'REVIEW' ? '완료 확인' : issue.workflowSummary.teamWorkCount === 0 ? '팀 작업 시작' : issue.workflowSummary.unassignedCount ? '담당자 지정' : '업무 보기';
  ...
  return (
    <li className="group border-b last:border-b-0">
      <div className="grid min-h-16 grid-cols-[minmax(18rem,1fr)_8.5rem_7.5rem_10rem_6.5rem_5rem_7rem] items-center gap-3 px-3 py-2.5 text-sm max-xl:grid-cols-[minmax(18rem,1fr)_8.5rem_7.5rem_9rem_6.5rem_6rem] max-lg:grid-cols-[minmax(18rem,1fr)_8.5rem_7.5rem_6rem] max-md:grid-cols-1 max-md:gap-1 max-md:px-3">
        ...
        <span className="text-muted-foreground truncate max-lg:hidden">{issue.workflowSummary.teamWorkCount ? `작업 ${issue.workflowSummary.teamWorkCount}개` : '아직 없음'}</span>
        <span className="flex items-center gap-2 tabular-nums"><span>{issue.progress.percentage}%</span><Progress className="hidden w-12 xl:block" value={issue.progress.percentage} /></span>
        <time className="text-muted-foreground text-xs max-xl:hidden" dateTime={issue.updatedAt}>{relativeUpdatedAt(issue.updatedAt)}</time>
        <Link href={`/issues/${encodeURIComponent(issue.identifier)}?tab=work`} className="text-muted-foreground hover:text-foreground inline-flex items-center gap-1 text-xs font-medium max-lg:hidden">{issue.workflowSummary.unassignedCount ? <CircleAlert className="text-warning size-3.5" /> : <ArrowRight className="size-3.5" />}{nextAction}</Link>
      </div>
    </li>
  );
}
