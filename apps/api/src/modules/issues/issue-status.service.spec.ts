...
describe('IssueStatusService.recalculate', () => {
  ...
  it('팀 작업이 없으면 UNSORTED로 계산하고 0% 문맥을 유지한다', async () => {
    ...
    const next = await service.recalculate(transaction as never, 'workspace-1', 'issue-1');
  ...
  it('시작 전 팀 작업만 있으면 TODO, 시작된 작업이 섞이면 IN_PROGRESS로 계산한다', async () => {
    ...
    expect(await service.recalculate(todoCase.transaction as never, 'workspace-1', 'issue-1')).toBe(
      IssueStatus.TODO,
    );

    const progressCase = transactionWith(IssueStatus.TODO, [unstarted(), started()]);
    expect(
      await service.recalculate(progressCase.transaction as never, 'workspace-1', 'issue-1'),
